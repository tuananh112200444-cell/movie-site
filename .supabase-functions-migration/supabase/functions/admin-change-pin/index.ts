...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: rateRecord, error: rateError } = await supabase.from('admin_login_attempts').select('change_attempt_count, change_locked_until').eq('ip_hash', ipHash).maybeSingle();
    if (rateError) {
    ...
    if (rateRecord?.change_locked_until) {
      ...
      if (lockTime > now) {
        ...
        return json({
          error: `Change password is locked for ${minsLeft} minutes.`
        }, 429, corsHeaders);
      }
    }
    // Check if PIN exists in DB
    const { data: setting, error: settingError } = await supabase.from('admin_settings').select('pin').eq('id', 1).maybeSingle();
    if (settingError) {
    ...
    if (action === 'setup' && hasExistingPin) {
      return json({
        error: 'Admin password already exists. Please log in or use change password.'
      }, 409, corsHeaders);
    }
    // ── SETUP MODE (no existing PIN) ─────────────────────────────
    if (action === 'setup' || !hasExistingPin) {
      if (!newPin) return json({
        error: 'Missing new password.'
      }, 400, corsHeaders);
      const complexity = validatePinComplexity(newPin);
      if (!complexity.valid) return json({
    ...
    if (!currentPin || !newPin) {
      return json({
        error: 'Missing current password or new password.'
      }, 400, corsHeaders);
    }
    ...
    if (hashedCurrent !== actualPinHash) {
      ...
      const changeLockedUntil = newChangeCount >= MAX_CHANGE_ATTEMPTS ? new Date(now + CHANGE_LOCK_MS).toISOString() : rateRecord?.change_locked_until;
      ...
      if (newChangeCount >= MAX_CHANGE_ATTEMPTS) {
        return json({
          error: 'Too many wrong current passwords. Change password is locked for 10 minutes.'
        }, 429, corsHeaders);
      }
      return json({
        error: `Current password is wrong. ${remaining} attempts left.`
      }, 401, corsHeaders);
    }
    ...
    return json({
      error: message,
      message: 'Cannot update admin password right now. Please try again later.'
    }, 503, corsHeaders);
  }
});
