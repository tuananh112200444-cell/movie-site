...
function safePathPart(value, fallback) {
  const cleaned = value.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/đ/g, 'd').replace(/[^a-z0-9._-]+/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '');
...
function normalizeVttText(text) {
  const normalized = text.replace(/^\uFEFF/, '').replace(/\r\n/g, '\n').replace(/\r/g, '\n').replace(/^\s*WEBVTT[^\n]*\n*/i, '').replace(/^\s*\d+\s*\n(?=\d{2}:\d{2}:\d{2}[,.]\d{3}\s+-->\s+)/gm, '').replace(/(\d{2}:\d{2}:\d{2}),(\d{3})/g, '$1.$2').trim();
  return `WEBVTT\n\n${normalized}\n`;
}
...
function cleanEpisodePayload(episode) {
  const cleaned = {
    ...episode
  };
...
function sourceText(movie) {
  return `${movie.source_site || ''} ${movie.source_name || ''}`.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/\u0111/g, 'd').replace(/\u0110/g, 'd').replace(/[^a-z0-9\s_-]/g, ' ').replace(/[_-]+/g, ' ').replace(/\s+/g, ' ').trim();
}
function isQueerSource(movie) {
...
function sameYearOrUnknown(a, b) {
...
async function countMovieEpisodes(supabase, movieId) {
  const { count } = await supabase.from('movie_episodes').select('id', {
    count: 'exact',
    head: true
  }).eq('movie_id', movieId).neq('source', 'hidden');
  return count || 0;
}
async function findCanonicalMovie(supabase, movieId) {
  if (!movieId) return {
    id: movieId,
    slug: '',
    changed: false
  };
  const { data: current } = await supabase.from('movies').select(MOVIE_CANONICAL_SELECT).eq('id', movieId).maybeSingle();
  if (!current) return {
    id: movieId,
    slug: '',
    changed: false
  ...
  const exactChecks = [
    ...
    {
      column: 'ophim_slug',
      value: currentMovie.ophim_slug || ''
    },
    {
      column: 'ophim_id',
      value: currentMovie.ophim_id || ''
    },
    {
      column: 'imdb_id',
      value: currentMovie.imdb_id || ''
    },
    {
      column: 'tmdb_id',
      value: currentMovie.tmdb_id || ''
    }
  ].filter((item)=>String(item.value || '').trim());
  for (const check of exactChecks){
    const { data } = await supabase.from('movies').select(MOVIE_CANONICAL_SELECT).eq(check.column, check.value).limit(10);
  ...
  for (const term of rawTerms){
    ...
    let query = supabase.from('movies').select(MOVIE_CANONICAL_SELECT).or(`name.ilike.%${safeTerm}%,title_vi.ilike.%${safeTerm}%,title_en.ilike.%${safeTerm}%,title_zh.ilike.%${safeTerm}%,title_original.ilike.%${safeTerm}%,origin_name.ilike.%${safeTerm}%,slug.ilike.%${safeTerm}%,ophim_slug.ilike.%${safeTerm}%`);
...
async function canonicalizeEpisodePayload(supabase, episode) {
...
async function uploadSubtitleIfPresent(supabase, episode) {
  ...
  const { error } = await supabase.storage.from(SUBTITLE_BUCKET).upload(path, uploadBytes, {
    cacheControl: '3600',
    contentType,
    upsert: true
  });
...
serve(async (req)=>{
  ...
  try {
    ...
    if (body.action === 'update' && body.id) {
      ...
      const { data, error } = await supabase.from('movie_episodes').update(updateData).eq('id', body.id).select().single();
      if (error) {
        return new Response(JSON.stringify({
          error: error.message
        }), {
    ...
    if (body.action === 'delete' && body.id) {
      const { error } = await supabase.from('movie_episodes').delete().eq('id', body.id);
      if (error) {
        return new Response(JSON.stringify({
          error: error.message
        }), {
    ...
    if (movieId && serverName && episodeNumber !== null && !Number.isNaN(episodeNumber)) {
      let existing = null;
      if (source !== 'hidden') {
        const { data } = await supabase.from('movie_episodes').select('id, source').eq('movie_id', movieId).eq('server_name', serverName).eq('episode_number', episodeNumber).eq('source', 'hidden').limit(1).maybeSingle();
        existing = data;
      }
      if (!existing) {
        let existingQuery = supabase.from('movie_episodes').select('id, source').eq('movie_id', movieId).eq('server_name', serverName).eq('episode_number', episodeNumber);
        if (source === 'hidden') {
      ...
      if (existing) {
        ...
        const { data, error } = await supabase.from('movie_episodes').update(updateData).eq('id', existing.id).select().single();
        if (error) {
          return new Response(JSON.stringify({
            error: error.message
          }), {
    ...
    // Insert
    const { data, error } = await supabase.from('movie_episodes').insert(incoming).select().single();
    if (error) {
      return new Response(JSON.stringify({
        error: error.message
      }), {
