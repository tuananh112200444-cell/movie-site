...
async function fetchFreshIndex(supabase, limit) {
  const { error } = await supabase.rpc('refresh_search_index_cache', {
    p_limit: limit
...
function readCachedItems(cacheRow) {
  const sections = cacheRow?.sections;
  if (!sections || typeof sections !== 'object') return [];
  const items = sections.items;
  return Array.isArray(items) ? items : [];
}
async function readCachedRows(supabase, limit) {
  const items = [];
  const pageSize = 1000;
  for(let from = 0; from < limit; from += pageSize){
    const to = Math.min(from + pageSize - 1, limit - 1);
    const { data, error } = await supabase.from('search_index_cache_items').select('item').order('rank', {
      ascending: true
    }).range(from, to).abortSignal(timeoutSignal(12000));
    if (error) return {
      items,
      error: error.message
    };
    const rows = (data ?? []).map((row)=>row.item);
    items.push(...rows);
...
function isRefreshLocked(cacheRow) {
...
async function lockRefresh(supabase, cacheRow) {
  const sections = cacheRow?.sections ?? {};
...
serve(async (req)=>{
  ...
  try {
    const { data, error } = await supabase.from('home_page_cache').select('sections, updated_at, expires_at').eq('id', CACHE_ID).abortSignal(timeoutSignal(8000)).maybeSingle();
  ...
  const cacheRecentlyRebuilt = Boolean(cacheRow && cacheMetaCount >= Math.min(limit, 800) && now.getTime() - parsePostgresTimestamp(cacheRow.updated_at) < FORCE_REFRESH_COOLDOWN_MS);
  ...
  if (forceRefresh && cacheRecentlyRebuilt) {
    ...
    if (cached.items.length >= Math.min(limit, 100)) {
      return jsonResponse({
        status: true,
        source: 'refresh-cooled',
        items: cached.items.slice(0, limit),
        updated_at: cacheRow?.updated_at
      }, 200, cacheHeaders('HIT'));
    }
  }
    ...
    if (cached.items.length >= Math.min(limit, 100)) {
      ...
      if (!isRefreshLocked(cacheRow)) {
        try {
          ...
          runtime.EdgeRuntime?.waitUntil?.(fetch(refreshUrl.toString(), {
            headers: {
  ...
  return jsonResponse({
    ...
    cache_read_error: cacheReadError,
    refresh_error: fresh.error
  }, items.length > 0 ? 200 : 503, cacheHeaders(items.length > 0 ? 'MISS' : 'ERROR'));
}); /*
