...
Deno.serve(async (req)=>{
  ...
  if (!secret || req.headers.get('x-cron-secret') !== secret) return json({
    error: 'Unauthorized'
  }, 401);
  if (!SUPABASE_URL || !SERVICE_KEY) return json({
  ...
  const { data: snapshot } = await supabase.from('operations_health_snapshots').select('*').order('checked_at', {
    ascending: false
  }).limit(1).maybeSingle();
  if (!snapshot || snapshot.status === 'healthy') return json({
    ok: true,
    sent: false,
    reason: 'healthy'
  });
  ...
  const { data: notice, error: insertError } = await supabase.from('operations_alert_notifications').insert({
    snapshot_id: snapshot.id,
    severity: snapshot.status,
    fingerprint,
    status: 'pending'
  }).select('id').single();
  if (insertError) return json({
    error: insertError.message
  }, 500);
  if (!RESEND_KEY || !ADMIN_EMAIL) {
    await supabase.from('operations_alert_notifications').update({
      status: 'failed',
      error_message: 'Missing email configuration'
    }).eq('id', notice.id);
    return json({
  ...
  await supabase.from('operations_alert_notifications').update(response.ok ? {
    status: 'sent',
    provider_message_id: payload.id || null,
    sent_at: new Date().toISOString()
  } : {
    status: 'failed',
    error_message: payload.message || `HTTP ${response.status}`
  }).eq('id', notice.id);
  return json({
    ok: response.ok,
    sent: response.ok,
