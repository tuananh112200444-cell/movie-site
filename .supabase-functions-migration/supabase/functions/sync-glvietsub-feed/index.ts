...
const first = (value, pattern)=>decode(String(value || '').match(pattern)?.[1] || '').trim();
const slugify = (value = '')=>String(value).normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().replace(/đ/g, 'd').replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 120);
const meta = (html, key)=>first(html, new RegExp(`<meta[^>]+(?:property|name)=["']${key}["'][^>]+content=["']([^"']+)`, 'i'));
function titleAliases(...values) {
...
async function fetchText(url, timeout = 18_000, init = {}) {
...
function discoverDetailUrls(html, limit) {
  const urls = [];
  const seen = new Set();
...
function discoverSitemapUrls(xml) {
  return Array.from(xml.matchAll(/<loc>(https:\/\/www\.glvietsub\.net\/(?:phim-bo|phim-le)\/[^<]+)<\/loc>/gi)).map((match)=>decode(match[1]).replace(/\/$/, ''));
}
function episodeLinks(html) {
  const values = [];
  const byUrl = new Map();
  for (const match of html.matchAll(/<a[^>]+href=["'](https:\/\/www\.glvietsub\.net\/xem-phim\/[^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi)){
    ...
    const specialNumber = special ? Number(slugify(label).match(/dac-biet-(\d+)/i)?.[1] || sourceSlug.match(/-tap-dac-biet-(\d+)(?:-|$)/i)?.[1] || 0) : 0;
    const episode = {
      url,
  ...
  const usedSpecialNumbers = new Set(values.filter((episode)=>episode.special && episode.number < -1000).map((episode)=>Math.abs(episode.number) - 1000).filter(Boolean));
...
async function parseDetail(sourceUrl) {
  ...
  const episodeGroups = await mapWithConcurrency(links.slice(0, 80), 4, async (episode)=>{
    ...
    const serverCount = [
      ...episodeHtml.matchAll(/class=["']dooplay_player_option["']/gi)
    ].length;
    const directRawEmbed = episode.raw ? first(episodeHtml, /class=["'][^"']*no-video[^"']*["'][\s\S]{0,1200}?<iframe[^>]+src=["']([^"']+)/i) : '';
    const urls = postId && serverCount ? await playerUrls(postId, type, serverCount) : /^https?:\/\//i.test(directRawEmbed) ? [
      decode(directRawEmbed)
    ] : [];
    return urls.map((url, index)=>({
        episode_number: episode.number,
        episode_name: episode.special ? `${episode.label.replace(/\s*RAW\b/i, '').trim() || 'Tập Đặc Biệt'}${episode.raw ? ' RAW' : ''}` : `Tập ${episode.number}${episode.raw ? ' RAW' : ''}`,
        slug: episode.slug,
  ...
  const episodes = episodeGroups.flat();
  ...
  return {
    ...
    category: /phim-bach-hop|Bách Hợp/i.test(html) ? [
      {
        name: 'Bách Hợp',
        slug: 'bach-hop'
      }
    ...
    country: /phim-thai-lan/i.test(html) ? [
      {
        name: 'Thái Lan',
        slug: 'thai-lan'
      }
...
async function findMovie(db, entry) {
  ...
  const aliases = Array.from(new Set([
    entry.name,
    entry.originName
  ].map((value)=>plain(String(value || '')).trim()).filter(Boolean)));
...
async function storeEntryLegacy(db, entry) {
  let movie = await findMovie(db, entry);
  let created = false;
  const now = new Date().toISOString();
  if (!movie) {
    const payload = {
      slug: `glvietsub-${entry.sourceSlug}`,
      name: entry.name,
      ...
      quality: 'HD',
      lang: 'Vietsub',
      episode_current: `Tập ${entry.currentEpisode}`,
      episode_total: String(entry.expectedEpisodes || entry.currentEpisode),
      ...
      source_site: SOURCE,
      source_name: 'GLVietsub',
      is_published: Number(entry.currentEpisode || 0) > 0,
      last_synced_at: now,
    ...
    if (movie.source_site === SOURCE) Object.assign(update, {
      ...
      category: entry.category,
      country: entry.country,
      episode_current: `Tập ${nextCurrent}`,
  ...
  for (const episode of entry.episodes){
    const payload = {
      ...
      source: SOURCE,
      is_backup: movie.source_site !== SOURCE,
      audio_type: episode.raw ? 'raw' : 'vietsub'
    };
    const { data: old } = await db.from('movie_episodes').select('id').eq('movie_id', movie.id).eq('episode_number', episode.episode_number).eq('server_name', episode.server_name).limit(1).maybeSingle();
    if (old?.id) await db.from('movie_episodes').update(payload).eq('id', old.id);
    else {
      const { error } = await db.from('movie_episodes').insert(payload);
      if (error) throw error;
    }
    const streamPayload = {
      movie_id: movie.id,
    ...
    const { data: stream } = await db.from('streams').select('id,stream_url,embed_url').eq('movie_id', movie.id).eq('source', SOURCE).eq('server_name', episode.server_name).eq('episode_slug', episode.slug).limit(1).maybeSingle();
    if (stream?.id) {
      const urlChanged = String(stream.stream_url || '') !== String(streamPayload.stream_url || '') || String(stream.embed_url || '') !== String(streamPayload.embed_url || '');
      const updatePayload = urlChanged ? streamPayload : Object.fromEntries(Object.entries(streamPayload).filter(([key])=>![
          'health_status',
          'failure_count',
          'last_error'
        ].includes(key)));
      ...
      const { error } = await db.from('streams').insert(streamPayload);
      if (error) throw error;
    }
    rows += 1;
  }
  const translatedEpisodes = entry.episodes.filter((episode)=>!episode.raw);
  const translatedEpisodeNumbers = Array.from(new Set(translatedEpisodes.map((episode)=>Number(episode.episode_number || 0)).filter((episodeNumber)=>Number.isFinite(episodeNumber) && episodeNumber !== 0)));
  if (translatedEpisodeNumbers.length) {
    const translatedEpisodeSlugs = Array.from(new Set(translatedEpisodes.map((episode)=>String(episode.slug || '').trim()).filter(Boolean)));
    const { error: staleEpisodeError } = await db.from('movie_episodes').delete().eq('movie_id', movie.id).eq('source', SOURCE).eq('audio_type', 'raw').in('episode_number', translatedEpisodeNumbers);
    if (staleEpisodeError) throw staleEpisodeError;
    if (translatedEpisodeSlugs.length) {
      const { error: staleStreamError } = await db.from('streams').delete().eq('movie_id', movie.id).eq('source', SOURCE).eq('audio_type', 'raw').in('episode_slug', translatedEpisodeSlugs);
      if (staleStreamError) throw staleStreamError;
    }
  }
  await db.from('movie_api_cache').update({
...
async function storeEntry(db, entry) {
  ...
  const hasPlayableEpisode = playableEpisode > 0 && entry.episodes.length > 0;
  ...
  const displayEpisode = nextCurrent > 0 ? `Tập ${nextCurrent}` : rawEpisode > 0 ? `Tập ${rawEpisode} RAW` : 'Đang cập nhật';
  ...
  const { error: seoError } = await db.rpc('refresh_movie_seo_quality', {
    p_movie_id: movie.id
  });
  if (seoError) throw seoError;
  await db.from('home_page_cache').update({
    expires_at: now
  }).in('id', [
    'homepage_v3',
...
serve(async (req)=>{
  ...
  const expected = [
    ...
    Deno.env.get('GLVIETSUB_SYNC_SECRET')
  ].map((v)=>String(v || '').trim()).filter(Boolean);
  const supplied = req.headers.get('x-cron-secret') || url.searchParams.get('secret') || '';
  if (!expected.length) return json({
    success: false,
    error: 'Sync authentication is not configured'
  }, 503);
  if (!expected.includes(supplied)) return json({
    success: false,
    error: 'Unauthorized'
  }, 401);
  const dbUrl = Deno.env.get('SUPABASE_URL') || '';
  const serviceKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') || '';
  if (!dbUrl || !serviceKey) return json({
    success: false,
    error: 'Missing Supabase env'
  }, 500);
  const db = createClient(dbUrl, serviceKey, {
    auth: {
  ...
  if (explicitSlug) {
    ...
    const archiveWindow = archiveCount > 0 ? Array.from({
      length: Math.min(archiveBatchSize, archiveCount)
    }, (_, index)=>archiveUrls[(normalizedBackfillOffset + index) % archiveCount]) : [];
  ...
  const parsedEntries = await mapWithConcurrency(discovered, 2, async (sourceUrl)=>{
    try {
      return {
        sourceUrl,
        entry: await parseDetail(sourceUrl),
        error: ''
      };
    } catch (error) {
      return {
  ...
  const result = {
    ...
    errors,
    circuit_open: consecutiveFailures >= 3,
    elapsed_ms: Date.now() - started
  };
  if (!explicitSlug && !dryRun) {
    const nextPage = archiveCount > 0 ? (normalizedBackfillOffset + archiveBatchSize) % archiveCount : 0;
    await db.from('sync_cursors').upsert({
      key: 'glvietsub-feed-backfill',
      page: nextPage,
  ...
  if (!dryRun) await db.from('sync_logs').insert({
    function_name: 'sync-glvietsub-feed',
    run_at: new Date().toISOString(),
    scanned: discovered.length,
    added: stored.filter((value)=>value.created).length,
    skipped: stored.filter((value)=>!value.created).length,
