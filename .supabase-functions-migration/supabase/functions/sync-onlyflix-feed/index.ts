...
function trendingMoviesFromHome(html, limit) {
  ...
  const selected = preferredPeriods.map((key)=>({
      period: key,
      rows: periods[key]
    })).find((candidate)=>Array.isArray(candidate.rows) && candidate.rows.length > 0);
  if (!selected) throw new Error('OnlyFlix Trending Movies has no published rows');
  return selected.rows.slice(0, limit).map((row, index)=>{
    ...
    return {
      ...
      link,
      title: {
        rendered: String(row.title || slug)
      },
...
async function playersFromPage(url, html) {
  ...
  const payload = JSON.parse(raw);
  const candidates = (Array.isArray(payload?.data?.players) ? payload.data.players : []).filter((item)=>/^https?:\/\//i.test(String(item?.url || '')));
  ...
  const healthyProviders = candidates.filter((item)=>/moviesapi\.to|vidfast\.(?:pro|vc)|multiembed\.mov/i.test(String(item.url || ''))).sort((a, b)=>Number(/moviesapi\.to/i.test(String(b.url || ''))) - Number(/moviesapi\.to/i.test(String(a.url || ''))) || Number(a.number || 99) - Number(b.number || 99));
...
async function parsePage(item) {
  ...
  const baseTitle = text(String(item.title?.rendered || meta(html, 'og:title') || item.slug || '')).replace(/\s+(?:watch free|online streaming).*$/i, '').trim();
  ...
  if (links.length) for (const link of links)groups.set(link.season, [
    ...groups.get(link.season) || [],
    link
  ]);
  else groups.set(0, [
    {
  ...
  for (const [season, episodes] of groups){
    ...
    for (const episode of episodes.slice(0, 60)){
      ...
      for (const player of players)playable.push({
        ...
        source_priority: /moviesapi\.to/i.test(String(player.url || '')) ? 25 : /vidfast\.(?:pro|vc)/i.test(String(player.url || '')) ? 15 : 10
      });
    }
    ...
    output.push({
      ...
      thumb: meta(html, 'og:image') || match(html, /(https:\/\/onlyflix\.to\/wp-content\/uploads\/posters\/[^"')]+)/i),
      poster: meta(html, 'twitter:image') || meta(html, 'og:image'),
      season,
      type: season ? 'series' : 'single',
      episodes: playable,
...
async function storeEntry(db, entry) {
  ...
  if (!movie) {
    const payload = {
      slug: `onlyflix-${entry.sourceSlug}`,
      name: entry.name,
      ...
      episode_total: String(entry.currentEpisode),
      current_episode: entry.currentEpisode,
      total_episodes: entry.currentEpisode,
    ...
    const hasVerifiedFullMovie = entry.type === 'single' && entry.episodes.some((episode)=>episode.slug === 'full');
    const pendingLabel = /^(?:trailer|sap chieu|sắp chiếu|coming soon|dang cap nhat)$/i.test(String(movie.episode_current || '').trim());
  ...
  for (const episode of entry.episodes){
    ...
    const streamPayload = {
      ...
      priority: Number(episode.source_priority || 15),
      is_active: true,
...
serve(async (req)=>{
  ...
  const expected = [
    ...
    Deno.env.get('ONLYFLIX_SYNC_SECRET')
  ...
  for (const item of discovered){
    ...
    try {
      const entries = await parsePage(item);
      parsed += entries.length;
      for (const entry of entries){
        const saved = dryRun ? {
          slug: (await findMovie(db, entry))?.slug || `onlyflix-${entry.sourceSlug}`,
          name: entry.name,
          created: false,
  ...
  if (!dryRun) {
    await db.from('sync_logs').insert({
      function_name: 'sync-onlyflix-feed',
      run_at: new Date().toISOString(),
      scanned: discovered.length,
      added: stored.filter((x)=>x.created).length,
      skipped: stored.filter((x)=>!x.created).length,
    ...
    if (result.success) await db.from('home_page_cache').update({
      expires_at: new Date().toISOString()
    }).eq('id', 'homepage_v3');
  }
