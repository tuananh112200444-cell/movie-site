...
function slugify(value) {
  return value.normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/\u0111/g, 'd').replace(/\u0110/g, 'd').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 110);
}
...
function titleKeysFromMovie(movie) {
  return new Set([
    movie.name,
    movie.origin_name,
    movie.title_vi,
  ...
  ].map(compactTitle).filter((value)=>value.length >= 8));
}
...
function episodeNumber(value) {
  const matches = Array.from(String(value || '').matchAll(/\d+/g)).map((match)=>Number(match[0])).filter(Number.isFinite);
  return matches.length ? Math.max(...matches) : 0;
}
function hasPlayableMarker(movie) {
...
function toImage(path, size = 'w500') {
  return path ? `https://image.tmdb.org/t/p/${size}${path}` : '';
}
function toType(mediaType) {
  return mediaType === 'tv' ? 'phim-bo' : 'phim-le';
}
function toCatalogStatus(detail, mediaType) {
...
function toEpisodeCurrent(status) {
  return status === 'upcoming' ? 'Sap chieu' : 'Dang cap nhat';
}
function buildMoviePayload(detail, mediaType, windowStart, windowEnd) {
  ...
  const directors = (detail.credits?.crew ?? []).filter((item)=>item.job === 'Director' || mediaType === 'tv' && item.job === 'Creator').map((item)=>item.name).filter(Boolean).slice(0, 8);
  ...
  return {
    ...
    schedule_note: catalogStatus === 'upcoming' ? 'Phim sap chieu, KhoPhim se cap nhat khi co nguon xem.' : 'Phim dang duoc cap nhat.',
    tmdb_id: detail.id,
...
async function collectCandidates(pages, windowStart, windowEnd) {
  const candidates = [];
  ...
  for(let page = 1; page <= pages; page++){
    const [movieDiscover, tvDiscover, movieUpcoming, trending] = await Promise.all([
      ...
      page <= 2 ? tmdbFetch('/trending/all/week', {
        language: 'en-US',
        page
  ...
  const seen = new Set();
  return candidates.filter(({ item, mediaType })=>{
...
async function upsertCatalogMovie(supabase, payload) {
  ...
  if (payloadKeys.size > 0 && payloadYear > 0) {
    ...
    ].map(safeIlikeTerm).filter(Boolean).slice(0, 4);
    let query = supabase.from('movies').select('id,slug,name,origin_name,title_vi,title_en,title_original,episode_current,current_episode,source_site,is_published,year').eq('is_published', true).eq('year', payloadYear).neq('source_site', 'tmdb-catalog').limit(80);
    if (terms.length > 0) {
    ...
    playableDuplicate = (data ?? []).find((movie)=>{
  ...
  const existing = await supabase.from('movies').select('id,slug,episode_current,source_site,is_published').eq('tmdb_id', tmdbId).eq('tmdb_media_type', mediaType).limit(1).maybeSingle();
  ...
  if (existing.data?.id) {
    ...
    const hasPlayableMarker = existingSource !== 'tmdb-catalog' && episode && ![
      'trailer',
      'sap chieu',
      'dang cap nhat'
    ].includes(episode);
...
serve(async (req)=>{
  ...
  if (!CRON_SECRET) return jsonResponse({
    error: 'Sync authentication is not configured'
  }, 503);
  if (providedSecret !== CRON_SECRET) return jsonResponse({
    error: 'Unauthorized'
  }, 401);
  ...
  try {
    ...
    await supabase.from('home_page_cache').delete().in('id', [
      'search_index_v1'
    ]);
