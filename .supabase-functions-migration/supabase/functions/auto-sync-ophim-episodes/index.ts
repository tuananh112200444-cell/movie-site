...
function getMovieCurrentEpisode(movie) {
  const parse = (value = '')=>{
    ...
    const range = text.match(/(?:tap|ep|episode|táº­p)?\s*0*(\d{1,4})\s*[-â€“â€”]\s*0*(\d{1,4})/i);
  ...
  return Math.max(Number(movie.current_episode || 0), parse(movie.episode_current || ''));
}
function getTotalEpisodeNumber(value = '') {
  const text = String(value || '').toLowerCase();
  const slash = text.match(/(\d{1,4})\s*\/\s*(\d{1,4})/);
...
async function upsertNormalizedEpisodeSafely(supabase, row) {
  ...
  const { data: existing, error: lookupError } = await supabase.from('episodes').select('id').eq('movie_id', row.movie_id).ilike('server_name', String(row.server_name || '').trim()).ilike('episode_slug', String(row.episode_slug || '').trim()).limit(1).maybeSingle();
...
async function upsertStreamSafely(supabase, row) {
  ...
  if (!error) return null;
  if (!isDuplicateDbError(error)) return error.message;
  const { data: existing, error: lookupError } = await supabase.from('streams').select('id').eq('movie_id', row.movie_id).eq('source', String(row.source || '').trim()).eq('is_active', true).ilike('server_name', String(row.server_name || '').trim()).ilike('episode_slug', String(row.episode_slug || '').trim()).limit(1).maybeSingle();
  if (lookupError) return lookupError.message;
  if (!existing?.id) return null;
...
function shouldRoutineSyncMovie(movie, includeCompleted) {
...
function detailMatchesMovie(movie, detail) {
...
function parseOPhimDetail(data, fallbackSlug) {
...
async function fetchOPhimDetail(slug) {
  ...
  for (const base of OPHIM_MIRRORS){
    try {
      ...
      const detail = parseOPhimDetail(await apiRes.json(), slug);
...
async function findOPhimSlug(movie) {
...
function isLikelyOPhimMovie(movie) {
  const source = `${movie.source_site || ''} ${movie.source_name || ''}`.toLowerCase();
  return Boolean(String(movie.ophim_slug || '').trim() || String(movie.ophim_id || '').trim() || source.includes('ophim'));
}
async function resolveOPhimDetail(movie) {
...
async function logSyncResult(supabase, payload) {
  try {
...
serve(async (req)=>{
  ...
  // â”€â”€â”€ Secret check (cron security) â”€â”€â”€
  ...
  if (secret !== cronSecret) {
    return new Response(JSON.stringify({
      error: 'Unauthorized â€” invalid or missing cron secret'
    }), {
      status: 401,
      headers: {
        ...corsHeaders,
  ...
  try {
    // â”€â”€â”€ Fetch movies that have ophim_id â”€â”€â”€
    ...
    let moviesQuery = supabase.from('movies').select('id, ophim_id, ophim_slug, slug, name, origin_name, title_vi, title_en, year, type, episode_current, current_episode, total_episodes, status, source_site, source_name, showtimes, updated_at, last_synced_at').eq('is_published', true).order('last_synced_at', {
      ascending: true,
      nullsFirst: true
    }).limit(queryLimit);
    if (targetSlug) {
    ...
    const eligibleMovies = (movies || []).filter((movie)=>shouldRoutineSyncMovie(movie, includeCompleted)).slice(0, limit);
    ...
    // â”€â”€â”€ Process each movie â”€â”€â”€
    for (const movie of eligibleMovies){
      try {
        const detail = await resolveOPhimDetail(movie);
        if (!detail) {
          totalSkipped++;
        ...
        for (const server of detail.episodes){
          ...
          for (const ep of episodes){
            ...
            const epName = ep.name || `Táº­p ${episodeNumber}`;
            const epSlug = ep.slug || `tap-${episodeNumber}`;
            // â”€â”€â”€ Check if this OPhim episode already exists in movie_episodes â”€â”€â”€
            const { data: existing } = await supabase.from('movie_episodes').select('id').eq('movie_id', movie.id).eq('server_name', serverName).eq('episode_number', episodeNumber).maybeSingle();
            if (existing) {
              movieSkipped++;
              continue; // SKIP â€” never update existing episodes
            }
            // â”€â”€â”€ INSERT into movie_episodes (admin table) â”€â”€â”€
            ...
            // â”€â”€â”€ INSERT into episodes (normalized per-episode table) â”€â”€â”€
            ...
            // â”€â”€â”€ INSERT into streams (individual stream per row) â”€â”€â”€
        ...
        const previousCurrent = getMovieCurrentEpisode(movie);
        const previousLabelEpisode = getTotalEpisodeNumber(movie.episode_current || '');
        const sourceCurrent = Math.max(getMovieCurrentEpisode({
          ...movie,
          current_episode: 0,
          episode_current: detail.episodeCurrent
        }), maxEpisode);
        const previousLooksAheadOfSource = sourceCurrent > 0 && previousCurrent > sourceCurrent && Number(movie.total_episodes || 0) > 0 && previousCurrent > Math.max(Number(movie.total_episodes || 0), sourceCurrent);
        const currentEpisode = previousLooksAheadOfSource ? sourceCurrent : Math.max(previousCurrent, sourceCurrent);
        const sourceTotal = Math.max(getTotalEpisodeNumber(detail.episodeTotal), getTotalEpisodeNumber(detail.episodeCurrent), maxEpisode);
        const mergedTotalEpisode = Math.max(Number(movie.total_episodes || 0) || 0, sourceTotal, currentEpisode);
        const movieUpdate = {
        ...
        if (currentEpisode !== previousCurrent || currentEpisode > 0 && previousLabelEpisode !== currentEpisode) {
          movieUpdate.episode_current = `Táº­p ${currentEpisode}`;
          movieUpdate.current_episode = currentEpisode;
        }
        await supabase.from('movies').update(movieUpdate).eq('id', movie.id);
        // Be nice to OPhim API â€” small delay between movies
