...
async function auditHomeProxy(supabaseUrl, serviceKey) {
  ...
  if (!response.ok) {
    return {
      ...
      source: 'error',
      counts: Object.fromEntries(HOME_SECTIONS.map((section)=>[
          section,
          0
        ])),
      failures: [
        `home-proxy HTTP ${response.status}`
      ]
    };
  }
  ...
  const failures = HOME_SECTIONS.filter((section)=>Number(counts[section] || 0) < MIN_HOME_ITEMS).map((section)=>`${section} chi co ${counts[section] || 0} phim`);
  return {
...
async function auditSearchIndex(supabaseUrl, serviceKey) {
  const started = performance.now();
  ...
  const payload = await response.json();
  const rawCount = (Array.isArray(payload?.items) ? payload.items.length : payload?.items) ?? payload?.count ?? payload?.total ?? payload?.total_items ?? (Array.isArray(payload?.movies) ? payload.movies.length : undefined) ?? (Array.isArray(payload?.data) ? payload.data.length : undefined) ?? (Array.isArray(payload?.index) ? payload.index.length : undefined);
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const [{ data: movies, error: movieError }, homeAudit, searchAudit] = await Promise.all([
      supabase.from('movies').select('id,slug,name,origin_name,source_site,source_name,thumb_url,poster_url,episode_current,current_episode,updated_at').eq('is_published', true).order('updated_at', {
        ascending: false
      }).limit(movieLimit),
    ...
    const [episodeRows, adminEpisodeRows, streamRows] = movieIds.length > 0 ? await Promise.all([
      ...
      fetchRowsForMovieIds(movieIds, (ids, from, to)=>supabase.from('streams').select('movie_id,episode_slug,stream_url,embed_url,is_active').in('movie_id', ids).eq('is_active', true).range(from, to))
    ]) : [
      [],
      [],
      []
    ...
    for (const movie of movieRows){
      ...
      if (playable <= 0) noPlayable.push({
        ...movie,
        advertised,
        playable
      });
      else if (playable < advertised) episodeMismatches.push({
        ...movie,
        advertised,
        playable
      });
    }
    const penalty = (homeAudit.ok ? 0 : 24) + (searchAudit.ok ? 0 : 18) + Math.min(30, episodeMismatches.length * 5) + Math.min(18, noPlayable.length * 2) + Math.min(12, missingImages.length);
    return json({
      ok: true,
