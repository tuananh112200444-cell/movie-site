...
function isTransientCallFailure(call) {
  if (call.ok) return false;
  const text = JSON.stringify(call.result || '').toLowerCase();
  return call.status === 0 && (text.includes('signal has been aborted') || text.includes('timeout') || text.includes('timed out') || text.includes('fetch failed') || text.includes('network'));
}
...
function displayedEpisode(movie) {
  return Math.max(Number(movie.current_episode || 0) || 0, episodeNumberFromText(movie.episode_current));
}
function playableEpisodeNumber(row) {
  const hasLink = Boolean(String(row.link_m3u8 || row.stream_url || '').trim() || String(row.link_embed || row.embed_url || '').trim());
  if (!hasLink) return 0;
  return Math.max(Number(row.episode_number || 0) || 0, episodeNumberFromText(row.episode_slug), episodeNumberFromText(row.slug), episodeNumberFromText(row.episode_name));
}
function getProvider(movie) {
...
async function readCursorPage(supabase, key) {
...
async function queryRowsByMovieIds(supabase, table, select, movieIds) {
  const rows = [];
  for(let index = 0; index < movieIds.length; index += 25){
    const ids = movieIds.slice(index, index + 25);
    if (ids.length === 0) continue;
    const { data, error } = await supabase.from(table).select(select).in('movie_id', ids).limit(1000);
    if (error) throw new Error(`${table}: ${error.message}`);
    rows.push(...data || []);
  }
  return rows;
}
async function findEpisodeMismatchCandidates(supabase, options) {
  ...
  const { data, error } = await supabase.from('movies').select('id, slug, name, source_site, source_name, ophim_slug, episode_current, current_episode').eq('is_published', true).or('source_site.ilike.%ophim%,source_name.ilike.%ophim%,source_site.ilike.%phimapi%,source_name.ilike.%kkphim%,ophim_slug.not.is.null').order('updated_at', {
    ascending: true,
    nullsFirst: true
  }).range(from, to);
  ...
  const movies = (data || []).map((movie)=>({
      ...movie,
      displayed: displayedEpisode(movie)
  ...
  }).filter((movie)=>movie.repair_slug && movie.playable_episode < movie.displayed_episode && (!options.severeOnly || movie.playable_episode <= 1)).sort((a, b)=>b.displayed_episode - b.playable_episode - (a.displayed_episode - a.playable_episode)).slice(0, options.repairLimit);
...
async function findMetadataAnomalies(supabase, options) {
  if (options.limit <= 0) return {
    anomalies: [],
    scanned: 0
  };
  const { data, error } = await supabase.from('movies').select('id, slug, name, source_site, source_name, ophim_slug, episode_current, current_episode').eq('is_published', true).or('source_site.ilike.%ophim%,source_name.ilike.%ophim%,source_site.ilike.%phimapi%,source_name.ilike.%kkphim%,ophim_slug.not.is.null').gte('current_episode', options.displayedThreshold).order('updated_at', {
    ascending: false,
    nullsFirst: false
  ...
  const anomalies = movies.map((movie)=>{
    const playable = playableByMovie.get(movie.id) || 0;
  ...
  }).filter((movie)=>movie.displayed_episode >= options.displayedThreshold && movie.playable_episode > 0 && movie.playable_episode <= options.maxPlayableForAnomaly && movie.gap >= options.minGap).sort((a, b)=>b.gap - a.gap).slice(0, options.limit);
  return {
    anomalies,
    scanned: (data || []).length
  };
}
async function callFunction(baseUrl, serviceKey, secret, name, params, extraHeaders = {}, timeoutMs = 25000) {
...
serve(async (req)=>{
  ...
  if (!cronSecret && !blvietsubSecret) return json({
    success: false,
  ...
  const calls = [];
  let mismatchRepair = null;
  let metadataAnomalies = null;
  if (!mismatchOnly) for (const provider of [
    'ophim',
    'kkphim'
  ]){
  ...
  const nonCriticalFailed = calls.filter((call)=>!call.ok && (call.name === 'search-index-proxy' || call.name === 'home-proxy' || call.name === 'sync-ophim-movies' && isTransientCallFailure(call)));
  const failed = calls.filter((call)=>!call.ok && call.name !== 'search-index-proxy' && call.name !== 'home-proxy' && !(call.name === 'sync-ophim-movies' && isTransientCallFailure(call)));
  return json({
