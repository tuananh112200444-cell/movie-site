...
async function enrichSeoMovie(supabase, movie) {
  ...
  const [adminEpisodes, episodes, quality] = await Promise.all([
    ...
    supabase.from('movie_seo_quality_status').select('eligible_for_index,index_tier,quality_score,reasons,signals,checked_at,latest_episode_number,declared_total_episodes,episode_progress_percent,freshness_score,last_episode_change_at,next_episode_at').eq('movie_id', movieId).maybeSingle()
  ]);
  const rows = [
    ...adminEpisodes.data || [],
    ...episodes.data || []
...
Deno.serve(async (req)=>{
  ...
  for (const variant of variants){
    const { data, error } = await supabase.from('movies').select(MOVIE_FIELDS).eq('slug', variant).eq('is_published', true).maybeSingle();
    if (error) return json({
      status: false,
      message: error.message
    }, 500);
    if (!error && data?.slug) return json({
      status: true,
      movie: await enrichSeoMovie(supabase, data)
    });
  }
  for (const variant of variants){
    const { data, error } = await supabase.from('movies').select(MOVIE_FIELDS).eq('ophim_slug', variant).eq('is_published', true).maybeSingle();
    if (error) return json({
      status: false,
      message: error.message
    }, 500);
