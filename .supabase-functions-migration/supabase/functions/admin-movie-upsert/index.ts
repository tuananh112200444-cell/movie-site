...
async function verifyAdminToken(token) {
  try {
    ...
    return {
      valid: valid && expiresAt > Math.floor(Date.now() / 1000),
      expiresAt
    };
  } catch  {
    return {
...
function sourceText(value) {
  const record = value;
  return `${record.source_site || ''} ${record.source_name || ''}`.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/\u0111/g, 'd').replace(/\u0110/g, 'd').replace(/[^a-z0-9\s_-]/g, ' ').replace(/[_-]+/g, ' ').replace(/\s+/g, ' ').trim();
}
function isQueerSource(value) {
  const source = sourceText(value);
  return source.includes('admin queer') || source.includes('blvietsub') || source.includes('vu tru dam my') || source.includes('dam my') || source.includes('bach hop') || source.includes('boy love') || source.includes('girl love') || source.includes('thai bl') || /\bbl\b/.test(source) || /\bgl\b/.test(source);
}
...
function canonicalDuplicateTitle(value) {
  return normalizeTitle(value).replace(/\b(18|19|20)\d{2}\b/g, ' ').replace(/\b(tap|ep|episode|phan|season|trailer|vietsub|thuyet minh|long tieng|full|hd|fhd|4k)\b/g, ' ').replace(/\b\d+\b/g, ' ').trim().replace(/\s+/g, ' ');
}
function canonicalTitleCandidates(value) {
  const record = value;
  return Array.from(new Set([
    record.name,
    record.title_vi,
    ...
    record.origin_name,
    record.normalized_name
  ].map(canonicalDuplicateTitle).filter((title)=>title.length >= 6)));
}
function seasonSignature(value) {
  const record = value;
...
function hasSharedTitle(existing, payload) {
  const incomingTitles = titleCandidates(payload);
  const existingTitles = titleCandidates(existing);
  const strictMatch = incomingTitles.some((incoming)=>existingTitles.some((existingTitle)=>incoming === existingTitle || incoming.length >= 8 && existingTitle.includes(incoming) || existingTitle.length >= 8 && incoming.includes(existingTitle)));
  ...
  return incomingCanonical.some((incoming)=>existingCanonical.some((existingTitle)=>incoming === existingTitle || incoming.length >= 10 && existingTitle.includes(incoming) || existingTitle.length >= 10 && incoming.includes(existingTitle)));
}
function isLikelySameMovie(existing, payload) {
...
async function findExistingMovieByTitle(supabase, payload) {
  const year = movieYear(payload);
  const terms = rawTitleTerms(payload);
  for (const term of terms){
    const safeTerm = escapePostgrestIlike(term);
    let query = supabase.from('movies').select(MOVIE_MATCH_SELECT).or(`name.ilike.%${safeTerm}%,title_vi.ilike.%${safeTerm}%,title_en.ilike.%${safeTerm}%,title_zh.ilike.%${safeTerm}%,title_original.ilike.%${safeTerm}%,origin_name.ilike.%${safeTerm}%,slug.ilike.%${safeTerm}%,ophim_slug.ilike.%${safeTerm}%`);
    const { data } = await query.limit(10);
    const match = (data ?? []).find((item)=>isLikelySameMovie(item, payload));
    if (match) return match;
  }
  if (year > 0 && titleCandidates(payload).length > 0) {
    const { data } = await supabase.from('movies').select(MOVIE_MATCH_SELECT).eq('year', year).order('updated_at', {
      ascending: false
    }).limit(200);
    const match = (data ?? []).find((item)=>isLikelySameMovie(item, payload));
    if (match) return match;
...
async function findExistingMovie(supabase, payload) {
  ...
  if (tmdbId && !Number.isNaN(tmdbId)) {
    const { data } = await supabase.from('movies').select(MOVIE_MATCH_SELECT).eq('tmdb_id', tmdbId).maybeSingle();
    if (data) return data;
  }
  // Priority 2: exact canonical slug match
  const slug = payload.slug ? String(payload.slug).trim() : null;
  if (slug) {
    const { data } = await supabase.from('movies').select(MOVIE_MATCH_SELECT).eq('slug', slug).maybeSingle();
    if (data) return data;
  }
  // Priority 3: exact imdb_id match
  const imdbId = payload.imdb_id ? String(payload.imdb_id).trim() : null;
  if (imdbId) {
    const { data } = await supabase.from('movies').select(MOVIE_MATCH_SELECT).eq('imdb_id', imdbId).maybeSingle();
    if (data) return data;
  }
  // Priority 4: exact ophim_slug match
  const ophimSlug = payload.ophim_slug ? String(payload.ophim_slug).trim() : null;
  if (ophimSlug) {
    const { data } = await supabase.from('movies').select(MOVIE_MATCH_SELECT).eq('ophim_slug', ophimSlug).maybeSingle();
    if (data) return data;
  }
  // Priority 5: ophim_id match
  const ophimId = payload.ophim_id ? String(payload.ophim_id).trim() : null;
  if (ophimId) {
    const { data } = await supabase.from('movies').select(MOVIE_MATCH_SELECT).eq('ophim_id', ophimId).maybeSingle();
    if (data) return data;
  }
  ...
  if (normalizedName && year && !Number.isNaN(year)) {
    const { data } = await supabase.from('movies').select(MOVIE_MATCH_SELECT).eq('normalized_name', normalizedName).eq('year', year).limit(1).maybeSingle();
    if (data) return data;
  }
  ...
  if (originName && originName.length > 2 && year && !Number.isNaN(year)) {
    const { data } = await supabase.from('movies').select(MOVIE_MATCH_SELECT).ilike('origin_name', originName).eq('year', year).limit(1).maybeSingle();
    if (data) return data;
  }
...
function mergeMovieData(existing, incoming) {
  const merged = {
    ...incoming
  };
...
function chooseFilled(current, incoming) {
  return hasValue(current) ? current : incoming;
}
function mergeMovieFields(target, sources) {
  const update = {};
  ...
  for (const key of Object.keys(update)){
    if (!hasValue(update[key]) || update[key] === target[key]) {
      delete update[key];
    }
  }
...
async function moveMovieEpisodes(supabase, sourceId, targetId) {
  const { data: rows, error } = await supabase.from('movie_episodes').select('id,movie_id,server_name,episode_number').eq('movie_id', sourceId);
  if (error) throw new Error(`movie_episodes: ${error.message}`);
  let moved = 0;
  let deduped = 0;
  for (const row of rows ?? []){
    const { data: existing } = await supabase.from('movie_episodes').select('id').eq('movie_id', targetId).eq('server_name', row.server_name || '').eq('episode_number', Number(row.episode_number || 0)).limit(1).maybeSingle();
    if (existing?.id) {
...
async function moveEpisodes(supabase, sourceId, targetId) {
  const { data: rows, error } = await supabase.from('episodes').select('id,server_name,episode_slug').eq('movie_id', sourceId);
  if (error) throw new Error(`episodes: ${error.message}`);
  let moved = 0;
  let deduped = 0;
  for (const row of rows ?? []){
    const { data: existing } = await supabase.from('episodes').select('id').eq('movie_id', targetId).eq('server_name', row.server_name || '').eq('episode_slug', row.episode_slug || '').limit(1).maybeSingle();
    if (existing?.id) {
...
async function moveStreams(supabase, sourceId, targetId) {
  const { data: rows, error } = await supabase.from('streams').select('id,episode_slug,source,server_name').eq('movie_id', sourceId);
  if (error) throw new Error(`streams: ${error.message}`);
  let moved = 0;
  let deduped = 0;
  for (const row of rows ?? []){
    const { data: existing } = await supabase.from('streams').select('id').eq('movie_id', targetId).eq('episode_slug', row.episode_slug || '').eq('source', row.source || '').eq('server_name', row.server_name || '').limit(1).maybeSingle();
    if (existing?.id) {
...
async function moveSimpleTable(supabase, table, sourceId, targetId, withUpdatedAt = true) {
  const updatePayload = {
    movie_id: targetId
  };
  if (withUpdatedAt) updatePayload.updated_at = new Date().toISOString();
  const { data, error } = await supabase.from(table).update(updatePayload).eq('movie_id', sourceId).select('id');
  if (error) throw new Error(`${table}: ${error.message}`);
  return (data ?? []).length;
}
async function cleanupCaches(supabase, slugs) {
...
async function mergeDuplicateMovies(supabase, targetId, sourceIds) {
  ...
  const { data: rows, error: fetchError } = await supabase.from('movies').select(MOVIE_MERGE_SELECT).in('id', ids);
  if (fetchError) throw new Error(`movies: ${fetchError.message}`);
  const movies = rows ?? [];
  const target = movies.find((movie)=>movie.id === targetId);
  const sources = uniqueSourceIds.map((id)=>movies.find((movie)=>movie.id === id)).filter((movie)=>Boolean(movie));
...
serve(async (req)=>{
  ...
  try {
    ...
    if (body.action === 'merge') {
      const result = await mergeDuplicateMovies(supabase, String(body.target_id || ''), Array.isArray(body.source_ids) ? body.source_ids.map(String) : []);
      return new Response(JSON.stringify({
        success: true,
        action: 'merge',
        ...result
    ...
    if (body.action === 'update' && body.id) {
      ...
      const { data, error } = await supabase.from('movies').update(updateData).eq('id', body.id).select('id, slug, name, poster_url, thumb_url, type').single();
      if (error) {
        return new Response(JSON.stringify({
          error: error.message
        }), {
    ...
    if (existing) {
      ...
      const { data, error } = await supabase.from('movies').update(updatePayload).eq('id', existing.id).select('id, slug, name, poster_url, thumb_url, type').single();
      if (error) {
        return new Response(JSON.stringify({
          error: error.message
        }), {
    ...
    const { data, error } = await supabase.from('movies').insert(payload).select('id, slug, name, poster_url, thumb_url, type').single();
    if (error) {
      return new Response(JSON.stringify({
        error: error.message
      }), {
