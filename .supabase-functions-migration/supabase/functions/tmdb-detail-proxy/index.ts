...
serve(async (req)=>{
  ...
  try {
    ...
    const res = await fetch(url.toString(), TMDB_READ_ACCESS_TOKEN ? {
      headers: {
    ...
    const crewArr = Array.isArray(credits.crew) ? credits.crew : [];
    const director = crewArr.find((c)=>c.job === 'Director')?.name ?? '';
    const productionCountries = Array.isArray(data.production_countries) ? data.production_countries.map((c)=>c.name) : [];
    const videoData = data.videos || {};
    const videoResults = Array.isArray(videoData.results) ? videoData.results : [];
