...
async function getPinHash(supabase) {
  const { data, error } = await supabase.from('admin_settings').select('pin').eq('id', 1).maybeSingle();
  if (error) {
...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: record, error: rateError } = await supabase.from('admin_login_attempts').select('attempt_count, last_attempt_at, locked_until').eq('ip_hash', ipHash).maybeSingle();
    if (rateError) {
    ...
    if (setup_required) {
      return json({
        setup_required: true,
        message: 'No admin password exists. Please create one.'
      }, 200, corsHeaders);
    }
    ...
    return json({
      error: message,
      message: 'Cannot check admin password right now. Please try again later.'
    }, 503, corsHeaders);
  }
});
