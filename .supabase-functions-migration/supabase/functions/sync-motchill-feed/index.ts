...
const reply = (body, status = 200)=>new Response(JSON.stringify(body, null, 2), {
    status,
    headers: {
      ...CORS,
...
const match = (value, pattern)=>decode(String(value || '').match(pattern)?.[1] || '').trim();
const slugify = (value = '')=>String(value).normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/\u0111/gi, 'd').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 120);
const meta = (html, key)=>match(html, new RegExp(`<meta[^>]+(?:property|name)=["']${key}["'][^>]+content=["']([^"']+)`, 'i'));
...
async function fetchText(url, timeout = 20_000, init = {}) {
...
function episodeNumber(value) {
  const values = Array.from(String(value || '').matchAll(/(?:tập|tap|episode|e)\s*[-.:]?\s*(\d{1,3})/gi)).map((item)=>Number(item[1])).filter((item)=>item > 0);
...
function baseSeasonTitle(value) {
...
function playerOptions(html) {
  ...
  for (const found of html.matchAll(/<li\b([^>]*class=["'][^"']*dooplay_player_option[^"']*["'][^>]*)>([\s\S]*?)<\/li>/gi)){
    ...
    if (type && post && nume) output.push({
      type,
      post,
      nume,
      label: text(found[2]) || `Server ${nume}`
    });
  }
...
function motchillServerName(option) {
  const audioType = inferAudioType(option.label);
  const audioLabel = audioType === 'thuyetminh' ? 'Thuyết Minh' : audioType === 'longtieng' ? 'Lồng Tiếng' : 'Vietsub';
  return `Motchill ${audioLabel} #${option.nume}`;
}
function embedUrlFromPayload(raw) {
  try {
...
async function fetchPlayer(option, episodeUrl) {
  ...
  if (!/^https?:\/\//i.test(url) || /youtube\.com|youtu\.be/i.test(url)) return {
    embedUrl: '',
    hlsUrl: ''
  };
  return {
    embedUrl: url,
    hlsUrl: await extractStreamcHls(url)
  };
}
async function extractStreamcHls(embedUrl) {
  try {
    const parsed = new URL(embedUrl);
    if (!/(^|\.)streamc\.xyz$/i.test(parsed.hostname)) return '';
    const html = await fetchText(embedUrl, 12_000, {
      headers: {
        Referer: BASE + '/',
...
async function parseEpisode(item) {
  ...
  for (const option of options){
    try {
      ...
      rows.push({
        ...
        episode_name: `Tập ${item.number}`,
        slug: `tap-${item.number}`,
        server_name: motchillServerName(option),
        link_embed: player.embedUrl,
        link_m3u8: player.hlsUrl,
        audio_type: inferAudioType(option.label),
        source_episode_url: item.url,
        quality: 'HD'
      });
    } catch  {}
  }
...
async function localizedTitle(db, sourceTitle, season) {
  ...
  let { data } = await db.from('movies').select(fields).ilike('origin_name', base).order('year', {
    ascending: false
  }).limit(1).maybeSingle();
  if (!data) ({ data } = await db.from('movies').select(fields).ilike('title_en', base).order('year', {
    ascending: false
  }).limit(1).maybeSingle());
...
async function parseMovie(db, sourceUrl) {
  const html = await fetchText(sourceUrl);
  const rawTitle = text(match(html, /<h1[^>]*>([\s\S]*?)<\/h1>/i) || meta(html, 'og:title')).replace(/\s*\(\d{4}\).*$/i, '').replace(/\s*-\s*(?:Full HD|Motchill).*$/i, '').trim();
  ...
  return {
    ...
    season,
    year,
    content: text(meta(html, 'description')),
    thumb: meta(html, 'og:image'),
    currentEpisode,
    totalEpisodes: Math.max(declared, currentEpisode),
    episodes: playable
  };
}
...
async function storeMovie(db, entry) {
  ...
  if (!movie) {
    const payload = {
      ...
      episode_total: String(entry.totalEpisodes),
    ...
    if (Number(entry.currentEpisode) >= current) Object.assign(update, {
      episode_current: `Tập ${entry.currentEpisode}`,
      current_episode: entry.currentEpisode,
      total_episodes: movie.source_site === SOURCE ? Number(entry.totalEpisodes || entry.currentEpisode) : Math.max(Number(movie.total_episodes || 0), Number(entry.totalEpisodes || 0))
    });
  ...
  for (const episode of entry.episodes){
    ...
    if (old?.id) await db.from('movie_episodes').update(episodePayload).eq('id', old.id);
      ...
      const { error } = await db.from('movie_episodes').insert(episodePayload);
      if (error) throw error;
    }
    const streamPayload = {
      movie_id: movie.id,
      ...
      stream_url: String(episode.link_m3u8 || ''),
    ...
    const { data: stream } = await db.from('streams').select('id,embed_url,stream_url').eq('movie_id', movie.id).eq('source', SOURCE).eq('server_name', episode.server_name).eq('episode_slug', episode.slug).limit(1).maybeSingle();
    if (stream?.id) {
      const changed = String(stream.embed_url || '') !== String(streamPayload.embed_url || '') || String(stream.stream_url || '') !== String(streamPayload.stream_url || '');
      const update = changed ? streamPayload : Object.fromEntries(Object.entries(streamPayload).filter(([key])=>![
          'health_status',
          'failure_count',
          'last_error'
        ].includes(key)));
  ...
  try {
    await db.rpc('refresh_movie_seo_quality', {
...
async function discover(query, limit) {
  if (query) {
    ...
    const searchResults = JSON.parse(raw).filter((item)=>[
        'tvshows',
        'movies',
        'posts'
      ].includes(String(item.subtype || ''))).map((item)=>normalizeSourceUrl(item.url)).filter(Boolean).slice(0, limit);
  ...
  const batches = await Promise.all([
    ...
    try {
      return JSON.parse(await fetchText(`${BASE}/wp-json/wp/v2/${kind}?per_page=${limit}&page=1&_fields=${fields}`));
    } catch  {
      return [];
    }
  }));
  return [
    ...new Set(batches.flat().sort((a, b)=>Date.parse(String(b.modified)) - Date.parse(String(a.modified))).map((item)=>normalizeSourceUrl(item.link)).filter(Boolean))
  ].slice(0, limit);
}
async function discoverOngoing(db, limit, claim) {
  if (claim) {
    const { data, error } = await db.rpc('claim_motchill_ongoing_for_sync', {
      p_limit: limit
    });
    if (error) throw error;
    return [
      ...new Set((data || []).map((item)=>normalizeSourceUrl(item.source_url)).filter(Boolean))
    ];
  }
  const { data, error } = await db.from('movies').select('source_url').eq('source_site', SOURCE).eq('is_published', true).eq('status', 'ongoing').not('source_url', 'is', null).order('last_synced_at', {
    ascending: true,
    nullsFirst: true
  }).limit(limit);
  if (error) throw error;
...
serve(async (req)=>{
  ...
  const urls = repairOngoing ? await discoverOngoing(db, limit, !dryRun) : await discover(query, limit);
  const stored = [];
  const errors = [];
  ...
  for (const sourceUrl of urls){
    try {
      ...
      stored.push(dryRun ? {
        name: entry.name,
        origin_name: entry.originName,
        episodes: entry.currentEpisode,
        sources: entry.episodes.length
      } : await storeMovie(db, entry));
    } catch (error) {
  ...
  const result = {
    ...
    dry_run: dryRun,
    mode: repairOngoing ? 'ongoing_recheck' : query ? 'title_repair' : 'new_releases',
    query,
    scanned: urls.length,
    stored,
  ...
  if (!dryRun) await db.from('sync_logs').insert({
    function_name: 'sync-motchill-feed',
    run_at: new Date().toISOString(),
    scanned: urls.length,
    added: stored.filter((item)=>item.created).length,
    skipped: stored.filter((item)=>!item.created).length + skippedUnplayable.length,
