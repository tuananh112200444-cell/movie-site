...
function streamUrl(row) {
  return String(row.stream_url || row.embed_url || '').trim();
}
function unique(items) {
  return Array.from(new Set(items));
}
function spreadAcrossMovies(rows, limit) {
  const onePerMovie = [];
  const overflow = [];
...
function isBrowserManagedPhimApiProbeBlocked(row, result) {
  const directUrl = String(row.stream_url || '').trim();
  const embedUrl = String(row.embed_url || '').trim();
  return Boolean(/https?:\/\/[^/]*(?:phim1280\.tv|kkphimplayer\d+\.com)\//i.test(directUrl) && /https?:\/\/player\.phimapi\.com\/player\//i.test(embedUrl) && (result.status === 401 || result.status === 403 || result.status === 404));
}
function scoreStream(row, responseMs) {
    ...
    if (isHls(url)) {
      ...
      if (!playlistText.includes('#EXTM3U')) {
        return {
          ...
          status: response.status,
          responseMs,
          error: 'HLS master playlist invalid'
        };
      }
      ...
      if (playlistText.includes('#EXT-X-STREAM-INF')) {
        ...
        if (!childPath) {
          return {
            ...
            status: response.status,
            responseMs,
            error: 'HLS master has no media playlist'
          };
        }
      ...
      if (!segmentPath) {
        return {
          ...
          status: response.status,
          responseMs: Date.now() - started,
          error: 'HLS playlist has no segment'
        };
      }
    ...
    if (/text\/html/i.test(contentType)) {
      ...
      if (/\b404\s+not\s+found\b|video\s+(?:was\s+)?(?:not\s+found|deleted|removed)|file\s+(?:was\s+)?(?:not\s+found|deleted|removed)/i.test(text)) {
        return {
          ...
          status: 404,
          responseMs,
          error: 'Embed returned an HTML 404/deleted-video page'
        };
      }
      if (/\b502\s+bad\s+gateway\b|\b503\s+service\s+unavailable\b|upstream\s+(?:connect\s+)?error/i.test(text)) {
        return {
          ...
          status: 502,
          responseMs,
          error: 'Embed returned an upstream gateway error page'
        };
      }
    }
...
async function probeStreamRow(row) {
  const directUrl = String(row.stream_url || '').trim();
  const embedUrl = String(row.embed_url || '').trim();
  ...
  const selectedFailure = failures.sort((a, b)=>{
    const severity = (status)=>status === 404 || status === 410 ? 4 : status !== null && status >= 500 ? 3 : status === 401 || status === 403 ? 2 : 1;
    return severity(b.status) - severity(a.status);
  })[0] ?? {
    ok: false,
    status: null,
    responseMs: 0,
    error: 'No stream or embed URL'
  };
  if (String(row.last_error || '').startsWith('Viewer telemetry:')) {
    return {
      ...selectedFailure,
...
async function logHealth(supabase, row, result) {
  ...
  await supabase.from('stream_health_logs').insert({
    ...
    error_message: browserManagedProbeBlocked ? 'Server probe blocked; browser validation required' : result.error,
    is_reachable: browserManagedProbeBlocked ? null : result.ok
  });
}
function healthStatusFor(result, failureCount) {
...
function shouldDeactivate(result, failureCount, deactivateAfter) {
...
async function updateStream(supabase, row, result, deactivateAfter) {
  const now = new Date().toISOString();
  const embedUrl = String(row.embed_url || '').trim();
  const browserManagedProbeBlocked = !String(row.stream_url || '').trim() && /https?:\/\/[^/]*streamc\.xyz\//i.test(embedUrl) && (result.status === 401 || result.status === 403) || isBrowserManagedPhimApiProbeBlocked(row, result);
  ...
  const telemetryEmbedCooldown = result.ok && !String(row.stream_url || '').trim() && String(row.embed_url || '').trim() && String(row.last_error || '').startsWith('Viewer telemetry:') && Number.isFinite(telemetryFailureAge) && telemetryFailureAge >= 0 && telemetryFailureAge < 30 * 60 * 1000;
  ...
  if (result.ok) {
    ...
    update.priority = result.directStreamFailed ? Math.min(Number(row.priority || 100), 40) : scoreStream(row, result.responseMs);
    ...
    update.priority = result.status === 401 || result.status === 403 ? Math.min(currentPriority, 35) : Math.max(1, currentPriority - (hardFail ? 80 : 35));
...
serve(async (req)=>{
  ...
  if (!authorized(req, url)) return json({
    success: false,
    error: 'Unauthorized'
  }, 401);
  ...
  let query = supabase.from('streams').select(streamSelect).eq('is_active', true).or('stream_url.neq.,embed_url.neq.').order('last_checked_at', {
    ascending: true,
    nullsFirst: true
  }).order('priority', {
    ascending: false
  }).limit(limit);
  let preselectedRows = null;
  if (slug) {
    ...
    const { data: targetMovie, error: targetMovieError } = await supabase.from('movies').select('id').eq('slug', slug).maybeSingle();
    if (targetMovieError) return json({
      success: false,
      error: targetMovieError.message
    }, 500);
    if (!targetMovie?.id) return json({
      success: true,
      checked: 0,
      results: [],
      message: 'Movie not found'
    });
    query = supabase.from('streams').select(streamSelect).eq('movie_id', targetMovie.id).or('stream_url.neq.,embed_url.neq.').order('last_checked_at', {
      ascending: true,
      nullsFirst: true
    }).order('priority', {
      ascending: false
    ...
    const [recentResult, backlogResult] = await Promise.all([
      supabase.from('streams').select(streamSelect).eq('is_active', true).eq('health_status', 'unchecked').or('stream_url.neq.,embed_url.neq.').order('updated_at', {
        ascending: false
      }).limit(Math.min(recentLimit * 4, 300)),
      backlogLimit > 0 ? supabase.from('streams').select(streamSelect).eq('is_active', true).eq('health_status', 'unchecked').or('stream_url.neq.,embed_url.neq.').order('last_checked_at', {
        ascending: true,
        nullsFirst: true
      }).order('priority', {
        ascending: false
      }).limit(Math.min(backlogLimit * 4, 300)) : Promise.resolve({
        data: [],
        error: null
      })
    ]);
    if (recentResult.error) return json({
      success: false,
      error: recentResult.error.message
    }, 500);
    if (backlogResult.error) return json({
      success: false,
      error: backlogResult.error.message
    }, 500);
    const distinctRows = [
      ...new Map([
        ...recentResult.data || [],
    ...
    // Hidden movies cannot generate new viewer telemetry, so they used to
    // starve behind the global dead-stream backlog. Probe one source from a
    // bounded mix of recently hidden and long-waiting movies. One verified
    // source is enough for the release-state trigger to restore the title.
    const recentMovieLimit = Math.ceil(movieLimit / 2);
    const backlogMovieLimit = Math.max(0, movieLimit - recentMovieLimit);
    const hiddenMovieQuery = ()=>supabase.from('movies').select('id,slug').eq('is_published', false).eq('seo_catalog_status', 'awaiting_playback');
    const [recentMoviesResult, backlogMoviesResult] = await Promise.all([
      hiddenMovieQuery().order('updated_at', {
        ascending: false
      }).limit(recentMovieLimit),
      backlogMovieLimit > 0 ? hiddenMovieQuery().order('updated_at', {
        ascending: true
      }).limit(backlogMovieLimit) : Promise.resolve({
        data: [],
        error: null
      })
    ]);
    const hiddenMovieError = recentMoviesResult.error || backlogMoviesResult.error;
    if (hiddenMovieError) return json({
      success: false,
      error: hiddenMovieError.message
    }, 500);
    const hiddenMovies = [
      ...new Map([
        ...recentMoviesResult.data || [],
        ...backlogMoviesResult.data || []
      ].map((movie)=>[
          String(movie.id),
          movie
        ])).values()
    ...
    if (hiddenMovieIds.length === 0) {
      ...
      const { data: recoveryRows, error: recoveryError } = await supabase.from('streams').select(streamSelect).in('movie_id', hiddenMovieIds).in('health_status', [
        'failed',
        'dead',
        'blocked'
      ]).or('stream_url.neq.,embed_url.neq.').order('last_checked_at', {
        ascending: true,
        nullsFirst: true
      }).order('priority', {
        ascending: false
      }).limit(Math.min(limit * 6, 600));
      if (recoveryError) return json({
        success: false,
        error: recoveryError.message
      }, 500);
      preselectedRows = spreadAcrossMovies(recoveryRows || [], limit);
    }
  } else if (queue === 'problem') {
    const { data: problemRows, error: problemError } = await supabase.from('streams').select(streamSelect).in('health_status', [
      'failed',
      'dead',
      'blocked'
    ]).or('stream_url.neq.,embed_url.neq.').order('last_checked_at', {
    ...
    query = supabase.from('streams').select(streamSelect).eq('is_active', true).or('last_checked_at.is.null,last_checked_at.lt.' + staleBefore).or('stream_url.neq.,embed_url.neq.').order('last_checked_at', {
      ascending: true,
      nullsFirst: true
    }).order('priority', {
      ascending: false
    ...
    const hotCandidateQuery = (pattern)=>supabase.from('streams').select(streamSelect).like('last_error', pattern).or(`last_checked_at.is.null,last_checked_at.lt.${hotRetryBefore}`).or('stream_url.neq.,embed_url.neq.').order('updated_at', {
        ascending: false
      }).limit(Math.min(limit * 4, 600));
    ...
    const telemetryRows = [
      ...new Map(prioritizedHotRows.map((row)=>[
          String(row.id),
          row
        ])).values()
    ].slice(0, limit);
    if ((telemetryRows || []).length > 0) {
      ...
      const { data: hotMovies, error: hotMovieError } = await supabase.from('movies').select('id,slug').eq('is_published', true).order('updated_at', {
        ascending: false
      }).limit(movieLimit);
      if (hotMovieError) return json({
        success: false,
        error: hotMovieError.message
      }, 500);
      const hotMovieRows = hotMovies || [];
      ...
      if (movieIds.length > 0) {
        ...
        const { data: freshRows, error: freshRowsError } = await supabase.rpc('pick_unchecked_stream_health_candidates', {
          p_movie_ids: movieIds,
          p_limit: limit
        });
        if (freshRowsError) return json({
          success: false,
          error: freshRowsError.message
        }, 500);
        const slugByMovieId = new Map(hotMovieRows.map((movie)=>[
            movie.id,
            String(movie.slug || '')
          ]));
        preselectedRows = (freshRows || []).map((row)=>({
            ...row,
            movies: {
              slug: slugByMovieId.get(row.movie_id) || ''
            }
          }));
      }
    }
  }
  const queryResult = preselectedRows ? {
  ...
  if (!dryRun) {
    const staleDetailSlugs = unique(results.filter((item)=>item && (!item.ok || item.degraded)).map((item)=>String(item?.movie_slug || '').trim()).filter(Boolean));
    for(let index = 0; index < staleDetailSlugs.length; index += 100){
      await supabase.from('movie_api_cache').delete().in('slug', staleDetailSlugs.slice(index, index + 100));
    }
  }
  return json({
    success: true,
