...
/** Extract and verify admin Bearer token */ /** Rate limit: 30 requests per IP per 60 seconds */ async function checkRateLimit(supabase, ipHash, endpoint) {
  const windowStart = new Date();
  windowStart.setSeconds(0, 0); // round to minute
  const { data: existing, error: fetchError } = await supabase.from('rate_limit_logs').select('*').eq('ip_hash', ipHash).eq('endpoint', endpoint).gte('window_start', windowStart.toISOString()).order('created_at', {
    ascending: false
  }).limit(1).maybeSingle();
  ...
  const { error: upsertError } = await supabase.from('rate_limit_logs').upsert({
    ip_hash: ipHash,
    endpoint,
...
/** Hash an IP string (simple DJB2 for rate limit keys) */ function hashIp(ip) {
  let hash = 5381;
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { error } = await supabase.from('movie_reviews').upsert(review, {
      onConflict: 'slug'
    });
