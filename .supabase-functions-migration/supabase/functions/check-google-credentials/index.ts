...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const keyValid = hasKey && key.includes('BEGIN PRIVATE KEY') && key.includes('END PRIVATE KEY');
    return new Response(JSON.stringify({
      ...
      message: isReady && keyValid ? 'Google Service Account credentials đã sẵn sàng' : !hasEmail ? 'Thiếu GOOGLE_SERVICE_ACCOUNT_EMAIL trong Supabase Secrets' : !hasKey ? 'Thiếu GOOGLE_SERVICE_ACCOUNT_KEY trong Supabase Secrets' : 'GOOGLE_SERVICE_ACCOUNT_KEY không đúng format (thiếu BEGIN/END PRIVATE KEY)'
    }), {
      headers: {
        'Content-Type': 'application/json',
        ...corsHeaders
