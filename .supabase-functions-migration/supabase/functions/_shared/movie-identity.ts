...
function canonicalPriority(movie) {
...
export async function findCanonicalMovieByIdentity(db, input) {
  ...
  for (const name of unique(input.names)){
    ...
    for (const column of [
      ...
      const { data, error } = await db.from('movies').select(fields).eq('year', year).ilike(column, exactCaseInsensitiveName).limit(20);
  ...
  const byId = new Map(candidates.filter((movie)=>movie?.id).map((movie)=>[
      String(movie.id),
      movie
    ]));
  return [
