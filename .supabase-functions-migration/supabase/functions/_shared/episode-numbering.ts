...
function advertisedTotal(movie) {
  const values = [
    movie.episode_total,
...
function explicitSeason(movie) {
  const tmdb = movie.tmdb && typeof movie.tmdb === 'object' ? movie.tmdb : null;
  ...
  const titleSeason = Number(title.match(/\b(?:season|part|phan|mua)\s*0*(\d{1,2})\b/)?.[1] || title.match(/\b0*(\d{1,2})\s*(?:season|part|phan|mua)\b/)?.[1] || 0);
  return titleSeason === tmdbSeason ? tmdbSeason : 0;
}
function canonicalEpisodeLabel(number) {
 ...
 */ export function normalizeVerifiedSeasonNumbering(movie, servers) {
  ...
  const numbers = Array.from(new Set(servers.flatMap((server)=>(server.server_data || []).map(episodeNumber)).filter((number)=>number > 0))).sort((a, b)=>a - b);
