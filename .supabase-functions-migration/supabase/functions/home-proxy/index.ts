...
/* ── CORS helpers ── */ const CORS_HEADERS = {
...
function homeCacheControl(maxAge = 60) {
...
function filteredSectionItems(sections, key) {
  return (sections?.[key] ?? []).filter((m)=>key === 'onlyflix-moi' || !isTrailerOnly(m.episode_current));
}
function cacheHasRequestedSections(sections, requestedSections) {
  if (!sections) return false;
  return requestedSections.every((key)=>filteredSectionItems(sections, key).length >= HOME_MIN_SECTION_ITEMS);
}
function buildPayloadFromSections(sections, requestedSections) {
  const payload = {};
...
async function readStaticHomeFallback(requestedSections) {
  try {
...
function mergeFreshWithStableCache(freshSections, cacheSections) {
  const merged = {};
  ...
  for (const key of keys){
    ...
    const cached = filteredSectionItems(cacheSections, key);
    merged[key] = fresh.length >= HOME_MIN_SECTION_ITEMS || cached.length < HOME_MIN_SECTION_ITEMS ? fresh : cached;
  }
  return merged;
}
...
function movieModifiedAt(item) {
  const raw = item.modified?.time ?? item.updated_at ?? item.last_episode_change_at ?? 0;
  const value = new Date(String(raw)).getTime();
  return Number.isFinite(value) ? value : 0;
}
async function fetchFreshEpisodeMovies(supabase, limit) {
  try {
    const { data: changes, error: changesError } = await supabase.from('movie_seo_quality_status').select('movie_id,last_episode_change_at').not('last_episode_change_at', 'is', null).gte('last_episode_change_at', new Date(Date.now() - HOME_FRESH_EPISODE_DAYS * 86400000).toISOString()).order('last_episode_change_at', {
      ascending: false,
      nullsFirst: false
    }).limit(limit).abortSignal(timeoutSignal(1200));
    if (changesError || !changes?.length) return [];
    const changedAtById = new Map();
    ...
    const { data: movies, error: moviesError } = await supabase.from('movies').select(`${HOME_SUPABASE_SELECT},tmdb_popularity,quality,lang`).in('id', ids).eq('is_published', true).not('poster_url', 'is', null).abortSignal(timeoutSignal(1400));
    if (moviesError || !movies?.length) return [];
    const currentYear = new Date().getFullYear();
    return movies.filter((movie)=>{
      const currentEpisode = Math.max(Number(movie.current_episode || 0) || 0, extractEpisodeNumber(movie.episode_current));
      const totalEpisodes = Math.max(Number(movie.total_episodes || 0) || 0, extractEpisodeNumber(movie.episode_total));
    ...
    }).filter(Boolean).filter((movie)=>!isTrailerOnly(movie.episode_current)).sort((a, b)=>movieModifiedAt(b) - movieModifiedAt(a)).slice(0, limit);
  } catch  {
...
/* ── Fetch with short timeout ── */ async function fetchOPhim(endpoint, timeoutMs = 3000) {
...
async function fetchExternal(endpoint, timeoutMs = 3000) {
...
function cleanMovieItem(raw, sourceSite = '') {
  ...
  return {
    ...
    tmdb_id: String(m.tmdb_id ?? m.tmdb?.id ?? ''),
...
function tmdbMediaType(item) {
  ...
  return [
    ...
    'tvshows',
    'hoathinh'
  ].includes(type) ? 'tv' : 'movie';
}
async function fetchVerifiedTmdbHeroArtwork(item) {
  ...
  try {
    ...
    if (existingId) {
      ...
      if (response.ok) selected = await response.json();
    } else if (queryTitle) {
...
async function enrichTrendingHeroArtwork(supabase, items) {
  const heroWindow = items.slice(0, 8);
  const heroSlugs = heroWindow.map((item)=>String(item.slug || '').trim()).filter(Boolean);
  const { data: verifiedRows } = heroSlugs.length ? await supabase.from('movies').select('slug,hero_backdrop_url,hero_poster_url').in('slug', heroSlugs).abortSignal(timeoutSignal(900)) : {
    data: []
  };
  const verifiedBySlug = new Map((verifiedRows ?? []).map((row)=>[
      String(row.slug),
      row
    ]));
  const enriched = await Promise.all(heroWindow.map(async (rawItem)=>{
    ...
    if (slug) {
      ...
      }).eq('slug', slug).abortSignal(timeoutSignal(900)).then(()=>undefined).catch(()=>undefined);
    }
    return next;
  }));
  return [
    ...enriched,
    ...items.slice(heroWindow.length)
  ];
}
function movieMatchKeys(movie) {
...
function overridePriority(movie) {
...
function applyMovieOverride(item, override) {
  return {
    ...item,
...
function extractEpisodeNumber(value) {
  ...
  const matches = [
    ...text.matchAll(/(\d{1,5})/g)
  ].map((match)=>Number(match[1])).filter(Number.isFinite);
  return matches.length ? Math.max(...matches) : 0;
}
function advertisedEpisode(item) {
  return Math.max(Number(item.current_episode || 0) || 0, extractEpisodeNumber(item.episode_current));
}
function capEpisodeToPlayable(item, playableMax) {
...
async function enrichWithPlayableEpisodeCounts(supabase, sections) {
  const ids = Array.from(new Set(Object.values(sections).flat().map((item)=>String(item._id || '')).filter((id)=>/^[0-9a-f-]{24,36}$/i.test(id))));
  if (ids.length === 0) return sections;
  const maxByMovieId = new Map();
  const setMax = (movieId, episodeNumber)=>{
  ...
  try {
    ...
    for(let i = 0; i < ids.length; i += chunkSize){
      ...
      const [movieEpisodes, episodes, streams] = await Promise.all([
        supabase.from('movie_episodes').select('movie_id, episode_number, slug, episode_name, link_m3u8, link_embed, source').in('movie_id', chunk).abortSignal(timeoutSignal(1200)),
        supabase.from('episodes').select('movie_id, episode_number, episode_slug, episode_name, link_m3u8, link_embed').in('movie_id', chunk).abortSignal(timeoutSignal(1200)),
        supabase.from('streams').select('movie_id, episode_slug, stream_url, embed_url, is_active').in('movie_id', chunk).eq('is_active', true).abortSignal(timeoutSignal(1200))
      ]);
      for (const row of movieEpisodes.data ?? []){
...
async function buildSupabaseOverrideMap(supabase, items) {
  ...
  try {
    ...
    for(let i = 0; i < slugs.length; i += chunkSize){
      ...
      if (bySlug) rows.push(...bySlug);
      if (byOphimSlug) rows.push(...byOphimSlug);
    }
    for(let i = 0; i < ophimIds.length; i += chunkSize){
      const chunk = ophimIds.slice(i, i + chunkSize);
      const { data } = await supabase.from('movies').select(HOME_OVERRIDE_SELECT).in('ophim_id', chunk).eq('is_published', true).abortSignal(timeoutSignal(1000));
      if (data) rows.push(...data);
    }
    if (years.length > 0) {
      const { data } = await supabase.from('movies').select(HOME_OVERRIDE_SELECT).in('year', years.slice(0, 12)).eq('is_published', true).order('updated_at', {
        ascending: false
      }).limit(500).abortSignal(timeoutSignal(1200));
      if (data) rows.push(...data);
    }
  } catch  {
    return null;
...
async function buildTrending(supabase, limit) {
  const [ophimNew, extNew, popularResult, freshEpisodeMovies] = await Promise.all([
    ...
    supabase.from('movies').select(`${HOME_SUPABASE_SELECT},tmdb_popularity,quality,lang`).eq('is_published', true).not('poster_url', 'is', null).gte('updated_at', new Date(Date.now() - 90 * 86400000).toISOString()).order('tmdb_popularity', {
      ascending: false,
      nullsFirst: false
    }).order('updated_at', {
      ascending: false,
      nullsFirst: false
    }).limit(limit * 3).abortSignal(timeoutSignal(1800)),
  ...
  for (const source of [
    ...
    {
      data: freshEpisodeMovies.length ? {
        items: freshEpisodeMovies
      } : null,
    ...
    for (const raw of extractItems(source.data)){
      ...
      if (!m) continue;
      if (isTrailerOnly(m.episode_current)) continue;
      ...
      if (!current || score > current.score) scored.set(key, {
        item: m,
        score
      });
    }
  ...
  const recent = freshUpstream.sort((a, b)=>movieModifiedAt(b) - movieModifiedAt(a));
  const result = [];
  const used = new Set();
  const add = (movie)=>{
    if (!movie) return;
...
function isAdultTop10Candidate(item) {
  const categoryText = (item.category ?? []).map((value)=>{
...
function top10FranchiseKey(item) {
  return normalizeTitle(item.origin_name || item.name || item.slug || '').replace(/\b(phan|season|movie|chapter|part)\s*\d+\b/g, '').replace(/\b\d{4}\b/g, '').trim().slice(0, 48);
}
async function fetchPlayableCandidateIds(supabase, ids) {
  const playable = new Set();
  const addRows = (rows, streamTable = false)=>{
    for (const row of rows ?? []){
      const hasLink = streamTable ? Boolean(String(row.stream_url || row.embed_url || '').trim()) : Boolean(String(row.link_m3u8 || row.link_embed || '').trim());
  ...
  for(let index = 0; index < ids.length; index += 80){
    ...
    const [movieEpisodes, episodes, streams] = await Promise.all([
      supabase.from('movie_episodes').select('movie_id,link_m3u8,link_embed,source').in('movie_id', chunk).abortSignal(timeoutSignal(1000)),
      supabase.from('episodes').select('movie_id,link_m3u8,link_embed').in('movie_id', chunk).abortSignal(timeoutSignal(1000)),
      supabase.from('streams').select('movie_id,stream_url,embed_url,is_active').in('movie_id', chunk).eq('is_active', true).abortSignal(timeoutSignal(1000))
    ]);
    addRows((movieEpisodes.data ?? []).filter((row)=>String(row.source || '').toLowerCase() !== 'hidden'));
    addRows(episodes.data ?? []);
    addRows(streams.data ?? [], true);
  }
  return playable;
}
async function buildTop10Singles(supabase, limit = 10, movieTypes = [
  'single',
  'phim-le'
]) {
  const currentYear = new Date().getFullYear();
  ...
  try {
    const baseQuery = ()=>supabase.from('movies').select(select).eq('is_published', true).in('type', movieTypes).not('poster_url', 'is', null);
    const [popularResult, viewedResult, recentResult] = await Promise.all([
      baseQuery().order('tmdb_popularity', {
        ascending: false,
        nullsFirst: false
      }).limit(60).abortSignal(timeoutSignal(1600)),
      baseQuery().gt('view', 0).order('view', {
        ascending: false,
        nullsFirst: false
      }).limit(60).abortSignal(timeoutSignal(1600)),
      baseQuery().gte('year', currentYear - 1).order('year', {
        ascending: false,
        nullsFirst: false
      }).order('tmdb_popularity', {
        ascending: false,
        nullsFirst: false
      }).limit(60).abortSignal(timeoutSignal(1600))
    ]);
    ...
    const [{ data: qualityRows }, directlyPlayableIds] = await Promise.all([
      supabase.from('movie_seo_quality_status').select('movie_id,quality_score,has_playable_episode,eligible_for_index,index_tier').in('movie_id', ids).eq('has_playable_episode', true).eq('eligible_for_index', true).abortSignal(timeoutSignal(1400)),
    ...
    const scored = Array.from(candidatesById.values()).filter((movie)=>!enforcePlayableGate || playableIds.has(String(movie.id || ''))).map((movie)=>{
      ...
      const year = Number(movie.year || 0);
      const recency = year >= currentYear ? 24 : year === currentYear - 1 ? 20 : year >= currentYear - 3 ? 15 : year >= currentYear - 8 ? 9 : 4;
      ...
      const score = Math.log1p(popularity) * 15 + Math.log1p(legacyViews) * 7.5 + recency + metadataQuality * 0.45 + (/(vietsub|thuyet minh|long tieng)/.test(language) ? 8 : 0) + (/(4k|fhd|hd)/.test(videoQuality) ? 5 : 0);
      return {
        movie,
        score
      };
    ...
    const add = (movie, enforceDiversity)=>{
      ...
      result.push(cleanMovieItem({
        ...
        _id: movie.id,
        modified: {
          time: movie.updated_at || new Date().toISOString()
        }
      }, String(movie.source_site || 'supabase-top10')));
    };
...
async function buildTop10Series(supabase, limit = 10) {
  return buildTop10Singles(supabase, limit, [
    'series',
    'phim-bo'
  ]);
}
/* ── Fetch a category / type list ── */ function supabaseTypeValues(section) {
  if (section === 'phim-le') return [
    'single',
    'phim-le'
  ];
  if (section === 'phim-bo') return [
    'series',
    'phim-bo'
  ];
  if (section === 'hoat-hinh') return [
    'hoathinh'
  ];
  if (section === 'phim-chieu-rap') return [
    'single'
  ];
  return [
    section
...
async function fetchSupabaseSection(supabase, typeOrCategory, isCountry = false, limit = 18) {
  try {
    let query = supabase.from('movies').select(HOME_SUPABASE_SELECT).eq('is_published', true);
    if (isCountry) {
      query = query.filter('country', 'cs', JSON.stringify([
        {
          slug: typeOrCategory
        }
      ]));
    ...
    const { data, error } = await query.order('updated_at', {
      ascending: false,
      nullsFirst: false
    }).limit(limit * 2).abortSignal(timeoutSignal(1500));
    if (error || !data) return [];
    return data.map((row)=>cleanMovieItem({
        ...row,
        _id: row.id,
        modified: {
          time: row.updated_at || new Date().toISOString()
        }
      }, 'supabase')).filter(Boolean).filter((m)=>!isTrailerOnly(m.episode_current)).slice(0, limit);
  } catch  {
    return [];
  }
}
async function fetchOnlyflixTrendingMovies(supabase, limit = 10) {
  try {
    const { data: logs, error: logError } = await supabase.from('sync_logs').select('metadata').eq('function_name', 'sync-onlyflix-feed').eq('success', true).order('run_at', {
      ascending: false
    ...
    .limit(8).abortSignal(timeoutSignal(1500));
    const trending = (logs ?? []).map((log)=>log.metadata?.trending).find((value)=>Array.isArray(value) && value.length > 0);
    if (logError || !trending) {
      ...
      const fallback = await readStaticHomeFallback([
        'onlyflix-moi'
      ]);
      return (fallback?.['onlyflix-moi'] ?? []).slice(0, Math.min(limit, 10));
    }
    const orderedSlugs = trending.map((row)=>String(row.slug || '')).filter(Boolean).slice(0, Math.min(limit, 10));
    if (!orderedSlugs.length) return [];
    const { data, error } = await supabase.from('movies').select(HOME_SUPABASE_SELECT).eq('is_published', true).in('slug', orderedSlugs).abortSignal(timeoutSignal(1500));
    if (error || !data) return [];
    const bySlug = new Map(data.map((row)=>[
        String(row.slug),
        row
      ]));
    return orderedSlugs.map((slug)=>bySlug.get(slug)).filter(Boolean).map((row)=>cleanMovieItem({
        ...row,
        _id: row.id,
        modified: {
          time: row.updated_at || new Date().toISOString()
        }
      }, 'onlyflix')).filter(Boolean).slice(0, Math.min(limit, 10));
  } catch  {
    return [];
  }
}
function mergeSectionWithPriority(priorityItems, sectionItems, limit) {
  const seen = new Set();
  const merged = [];
  for (const item of [
    ...priorityItems,
...
function mergeSectionWithReservedTail(sectionItems, supplementalItems, limit, supplementalSlots = 4) {
...
function mergeTrendingWithSourceDiversity(trendingItems, supplementalItems, limit) {
  if (supplementalItems.length === 0) return trendingItems.slice(0, limit);
  const seen = new Set();
  const result = [];
  const add = (item)=>{
    if (!item || isTrailerOnly(item.episode_current)) return;
  ...
  let turn = 0;
  while(result.length < limit && (primaryIndex < trendingItems.length || supplementalIndex < supplementalItems.length)){
...
function movieLooksAnimated(item) {
  const haystack = [
    item.type,
    item.name,
    item.origin_name,
...
async function fetchPlayableCobephimMovies(supabase, limit = 12) {
  try {
    const normalizeRows = (movies)=>movies.map((row)=>cleanMovieItem({
          ...row,
          _id: row.id,
          modified: {
            time: row.updated_at || new Date().toISOString()
        ...
        }, 'cobephim')).filter(Boolean).filter((movie)=>!isTrailerOnly(movie.episode_current));
    const { data: taggedMovies, error: taggedError } = await supabase.from('movies').select(HOME_SUPABASE_SELECT).eq('is_published', true).or('source_site.eq.cobephim,source_name.ilike.*CobePhim*,source_url.ilike.*cobephim*,showtimes.ilike.*cobephim*,source_url.ilike.*streamvsmov*,showtimes.ilike.*streamvsmov*').order('updated_at', {
      ascending: false,
      nullsFirst: false
    }).limit(limit * 2).abortSignal(timeoutSignal(1500));
    if (taggedError || !taggedMovies?.length) return [];
    return mergeSectionWithPriority(normalizeRows(taggedMovies), [], limit);
  } catch  {
    return [];
  }
}
async function fetchSection(supabase, typeOrCategory, isCountry = false, limit = 18) {
  const endpoint = isCountry ? `/v1/api/danh-sach/phim-moi-cap-nhat?country=${encodeURIComponent(typeOrCategory)}&page=1` : `/v1/api/danh-sach/${encodeURIComponent(typeOrCategory)}?page=1`;
  const externalEndpoint = isCountry ? `/danh-sach/phim-moi-cap-nhat?country=${encodeURIComponent(typeOrCategory)}&page=1` : endpoint;
  // OPhim ưu tiên
  const ophimData = await fetchOPhim(endpoint, 3000);
  if (ophimData) {
    const items = extractItems(ophimData).map((raw)=>cleanMovieItem(raw, 'ophim')).filter(Boolean).filter((m)=>!isTrailerOnly(m.episode_current)).slice(0, limit);
    if (items.length > 0) {
      return items;
    }
  }
  ...
  if (extData) {
    const items = extractItems(extData).map((raw)=>cleanMovieItem(raw, 'phimapi')).filter(Boolean).filter((m)=>!isTrailerOnly(m.episode_current)).slice(0, limit);
    return items;
  }
  return await fetchSupabaseSection(supabase, typeOrCategory, isCountry, limit);
}
/* ── Supabase custom overrides ── */ async function applySupabaseOverrides(supabase, items) {
  if (items.length === 0) return items;
  const slugs = items.map((m)=>m.slug).filter(Boolean);
  if (slugs.length === 0) return items;
  try {
    const { data: overrides } = await supabase.from('movies').select('slug, name, poster_url, thumb_url, hero_backdrop_url, hero_poster_url, episode_current, episode_total, current_episode, total_episodes, schedule_type, release_time, release_day, schedule_timezone, release_at, next_episode_at, next_episode_name, schedule_note, status').in('slug', slugs).eq('is_published', true);
    ...
    return items.map((item)=>{
      const ov = overrideMap.get(item.slug);
      if (!ov) return item;
      return {
        ...item,
...
/* ── Main ── */ serve(async (req)=>{
  /* Handle OPTIONS preflight immediately */ if (req.method === 'OPTIONS') {
  ...
  /* 1. Try cache first */ let cacheRow = null;
  let cacheReadUnavailable = false;
  try {
    const { data, error } = await supabase.from('home_page_cache').select('sections, updated_at, expires_at').eq('id', CACHE_KEY).abortSignal(timeoutSignal(1200)).maybeSingle();
    if (data) {
      cacheRow = data;
    }
  ...
  const cachedSections = cacheRow?.sections ?? undefined;
  const cacheCompleteForRequest = cacheHasRequestedSections(cachedSections, requestedSections);
  /* 2. If cache valid → return immediately (fast path <300ms) */ if (cacheValid && cacheCompleteForRequest && !forceRefresh) {
  ...
  /* 2b. Expired cache is still better than blocking first paint.
     Trigger a background refresh; the foreground request returns stale data fast. */ if (cacheRow && cacheEligibleForStale && cacheCompleteForRequest && !forceRefresh) {
    try {
      ...
      runtime.EdgeRuntime?.waitUntil?.((async ()=>{
        try {
          await supabase.from('home_page_cache').update({
            expires_at: lockExpiresAt
          }).eq('id', CACHE_KEY).abortSignal(timeoutSignal(800));
        } catch  {
  ...
  */ if (cacheReadUnavailable && !forceRefresh) {
  ...
  /* 3. Build requested sections in parallel with 3s timeout each */ const limit = 18;
  ...
  const sectionEntries = await Promise.all(entries.map(async ([key, promise])=>{
    try {
  ...
  /* 4. Apply Supabase overrides in one batch query */ const allFreshItems = Object.values(freshSections).flat();
  ...
  if (overrideMap) {
    for (const key of Object.keys(freshSections)){
      freshSections[key] = freshSections[key].map((item)=>{
        const ov = movieMatchKeys(item).map((matchKey)=>overrideMap.get(matchKey)).find((row)=>Boolean(row));
        if (!ov) return item;
        return applyMovieOverride(item, ov);
      });
    }
  }
  /* 4b. Filter out trailer-only movies from all sections */ for (const key of Object.keys(freshSections)){
    freshSections[key] = freshSections[key].filter((m)=>key === 'onlyflix-moi' || !isTrailerOnly(m.episode_current));
  }
  const playableCobephimMovies = await fetchPlayableCobephimMovies(supabase, 12);
  if (playableCobephimMovies.length > 0) {
    if (requestedSections.includes('trending')) {
      freshSections.trending = mergeTrendingWithSourceDiversity(freshSections.trending ?? [], playableCobephimMovies, limit);
    }
    if (requestedSections.includes('phim-le')) {
      freshSections['phim-le'] = mergeSectionWithReservedTail(freshSections['phim-le'] ?? [], playableCobephimMovies.filter((movie)=>[
          'single',
          'phim-le'
        ].includes(String(movie.type || '').toLowerCase())), limit);
    }
    if (requestedSections.includes('phim-bo')) {
      freshSections['phim-bo'] = mergeSectionWithReservedTail(freshSections['phim-bo'] ?? [], playableCobephimMovies.filter((movie)=>[
          'series',
          'phim-bo'
        ].includes(String(movie.type || '').toLowerCase())), limit);
    }
    if (requestedSections.includes('hoat-hinh')) {
      freshSections['hoat-hinh'] = mergeSectionWithReservedTail(freshSections['hoat-hinh'] ?? [], playableCobephimMovies.filter(movieLooksAnimated), limit);
    }
  }
  const safeFreshSections = await enrichWithPlayableEpisodeCounts(supabase, freshSections);
  /* 5. Stale-while-revalidate: if OPhim returned nothing but cache exists, return stale */ const hasAnyFresh = Object.values(safeFreshSections).some((arr)=>arr.length > 0);
  ...
  /* 6. Persist to cache.
     ...
     temp I/O on small Supabase instances. Updating the fixed cache row is cheaper. */ const responseSections = mergeFreshWithStableCache(safeFreshSections, cachedSections);
  try {
    ...
    const { data: updatedRows, error: updateError } = await supabase.from('home_page_cache').update(payload).eq('id', CACHE_KEY).select('id').abortSignal(timeoutSignal(3000));
    if (!updateError && (!updatedRows || updatedRows.length === 0)) {
      await supabase.from('home_page_cache').upsert({
        id: CACHE_KEY,
        ...payload
      }, {
        onConflict: 'id'
