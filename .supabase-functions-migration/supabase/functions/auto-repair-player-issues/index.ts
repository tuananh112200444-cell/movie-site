...
function getAuthSecret(req, url) {
...
function internalSecret(provided) {
...
function isOphimLikeMovie(movie) {
...
function getOphimRepairSlug(movie) {
...
async function penalizeTelemetryFailedStreams(supabase, movieId, hosts, episodeSlugs) {
  ...
  const { data } = await supabase.from('streams').select('id,episode_slug,stream_url,embed_url,failure_count,priority').eq('movie_id', movieId).eq('is_active', true).abortSignal(AbortSignal.timeout(12_000));
  let penalized = 0;
...
function summarizeCandidates(events, threshold, limit) {
  const map = new Map();
  ...
  return [
    ...map.values()
  ].filter((item)=>item.critical >= threshold).map((item)=>({
      ...item,
      hosts: item.hosts.filter((host)=>Number(item.host_counts[host] || 0) >= threshold)
    })).sort((a, b)=>b.score - a.score || b.critical - a.critical || b.last_seen_at.localeCompare(a.last_seen_at)).slice(0, limit);
}
function detectProviderIncidents(events) {
  const groups = new Map();
  for (const event of events){
    const host = String(event.source_host || '').toLowerCase().replace(/^www\./, '');
    const provider = /opstream|ophim/.test(host) ? 'ophim' : /kkphimplayer|phim1280|phimapi|kkphim/.test(host) ? 'kkphim' : null;
  ...
  return new Set([
    ...groups.entries()
  ].filter(([, item])=>item.hosts.size >= 3 && item.movies.size >= 20 && item.critical >= 30).map(([provider])=>provider));
}
async function callFunction(supabaseUrl, serviceKey, secret, name, params) {
...
async function deleteMovieCaches(supabase, slugs) {
...
serve(async (req)=>{
  ...
  if (req.method !== 'GET' && req.method !== 'POST') return json({
    success: false,
  ...
  if (!validSecret(providedSecret)) return json({
    success: false,
    error: 'Unauthorized'
  }, 401);
  ...
  const { data: eventRows, error: eventsError } = await supabase.from('player_error_events').select('movie_slug,movie_title,episode_slug,episode_name,server_name,event_type,source_host,player_mode,created_at').gte('created_at', since).in('event_type', [
    ...CRITICAL_EVENTS
  ]).order('created_at', {
    ascending: false
  }).limit(eventLimit).abortSignal(AbortSignal.timeout(15_000));
  if (eventsError) return json({
    success: false,
    error: eventsError.message
  }, 500);
  const playerEvents = eventRows || [];
  ...
  const { data: repairCursorRows } = repairKeys.length ? await supabase.from('sync_cursors').select('key,updated_at').in('key', repairKeys).abortSignal(AbortSignal.timeout(8_000)) : {
    data: []
  };
  const cooldownCutoff = Date.now() - cooldownMinutes * 60 * 1000;
  const recentRepairKeys = new Set((repairCursorRows || []).filter((row)=>Date.parse(String(row.updated_at || '')) >= cooldownCutoff).map((row)=>String(row.key || '')));
  ...
  const { data: movieRows, error: moviesError } = await supabase.from('movies').select('id,slug,name,origin_name,source_site,source_name,source_url,showtimes,ophim_id,ophim_slug').in('slug', slugs).abortSignal(AbortSignal.timeout(10_000));
  if (moviesError) return json({
    success: false,
    error: moviesError.message
  }, 500);
  const moviesBySlug = new Map((movieRows || []).map((movie)=>[
      movie.slug,
      movie
    ]));
  const repairs = [];
  ...
  for (const candidate of candidates){
    ...
    const penalizedStreams = await penalizeTelemetryFailedStreams(supabase, movie.id, candidate.hosts, candidate.episodes);
