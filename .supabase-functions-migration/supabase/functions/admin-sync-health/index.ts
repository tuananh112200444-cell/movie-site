...
function getEpisodeNumberFromText(value) {
...
function playableEpisodeNumber(row) {
  return Math.max(Number(row.episode_number || 0), getEpisodeNumberFromText(row.episode_name), getEpisodeNumberFromText(row.episode_slug), getEpisodeNumberFromText(row.slug));
}
...
function isTransientLog(row) {
  const text = `${stringifyLogValue(row.details)} ${stringifyLogValue(row.metadata)}`.toLowerCase();
  return text.includes('transient_error') || text.includes('error 521') || text.includes('"status":521') || text.includes('web server is down') || text.includes('cloudflare') || text.includes('timeout') || text.includes('timed out') || text.includes('signal has been aborted') || text.includes('fetch failed') || text.includes('rate limit') || text.includes('too many requests') || text.includes('502') || text.includes('503') || text.includes('504');
}
function isHardErrorLog(row) {
...
async function fetchPagedRows(makeQuery, maxRows = 50000, pageSize = 1000) {
  const rows = [];
...
async function fetchRowsForMovieIds(movieIds, makeQuery, batchSize = 10) {
  const rows = [];
  for(let index = 0; index < movieIds.length; index += batchSize){
    const ids = movieIds.slice(index, index + batchSize);
    rows.push(...await fetchPagedRows((from, to)=>makeQuery(ids, from, to)));
  }
  return rows;
}
function summarizeSyncLogs(logs) {
  const names = EXPECTED_SYNC_FUNCTIONS.map((function_name)=>({
      function_name,
      stale_hours: 6
    }));
  return names.map(({ function_name: name, stale_hours: staleHours })=>{
    ...
    return {
      ...
      status: !latest ? 'missing' : lastRunHoursAgo != null && lastRunHoursAgo > staleHours ? 'stale' : hardFailures >= 3 ? 'unstable' : isHardErrorLog(latest) ? 'failed' : failures > 0 ? 'warning' : 'ok'
    };
  });
}
function buildActionItems(syncStatus, mismatches, staleMovies) {
  const items = [];
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const [{ data: logs, error: logError }, { data: movies, error: movieError }] = await Promise.all([
      supabase.from('sync_logs').select('id,function_name,run_at,success,scanned,added,skipped,errors,elapsed_ms,details,metadata').order('run_at', {
        ascending: false
      }).limit(logLimit),
      supabase.from('movies').select('id,slug,name,origin_name,source_site,source_name,episode_current,current_episode,updated_at,last_synced_at').eq('is_published', true).gt('current_episode', 1).order('updated_at', {
        ascending: false
      }).limit(movieLimit)
    ]);
    if (logError) throw logError;
    if (movieError) throw movieError;
    const movieRows = (movies ?? []).filter((movie)=>getEpisodeNumber(movie) > 1);
    const movieIds = movieRows.map((movie)=>movie.id);
    let episodeRows = [];
    let adminEpisodeRows = [];
    let streamRows = [];
    if (movieIds.length > 0) {
      [episodeRows, adminEpisodeRows, streamRows] = await Promise.all([
        fetchRowsForMovieIds(movieIds, (ids, from, to)=>supabase.from('episodes').select('movie_id,episode_number,episode_name,episode_slug').in('movie_id', ids).gt('episode_number', 0).range(from, to)),
        fetchRowsForMovieIds(movieIds, (ids, from, to)=>supabase.from('movie_episodes').select('movie_id,episode_number,episode_name,slug').in('movie_id', ids).gt('episode_number', 0).range(from, to)),
        fetchRowsForMovieIds(movieIds, (ids, from, to)=>supabase.from('streams').select('movie_id,episode_slug').in('movie_id', ids).eq('is_active', true).range(from, to))
      ]);
    }
    const episodeMaxByMovie = new Map();
    for (const row of [
      ...episodeRows,
      ...adminEpisodeRows
    ]){
    ...
    const mismatches = movieRows.map((movie)=>{
    ...
    }).filter((item)=>item.expected_episode > item.actual_episode).sort((a, b)=>b.missing_count - a.missing_count || b.expected_episode - a.expected_episode).slice(0, 30);
    const staleMovies = movieRows.filter((movie)=>{
      const age = hoursAgo(movie.last_synced_at);
      return age == null || age > 24;
    }).sort((a, b)=>(hoursAgo(b.last_synced_at) ?? 9999) - (hoursAgo(a.last_synced_at) ?? 9999)).slice(0, 20);
    const syncStatus = summarizeSyncLogs(uniqueBy(logs ?? [], (row)=>`${row.function_name || ''}|${row.run_at || ''}|${row.id || ''}`));
    return json({
      ok: true,
