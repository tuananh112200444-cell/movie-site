...
const PROVIDERS = {
  ophim: {
    ...
    listPath: (page)=>`/v1/api/danh-sach/phim-moi-cap-nhat?page=${page}&sort_field=modified.time&sort_type=desc`,
    searchPath: (query)=>`/v1/api/tim-kiem?keyword=${encodeURIComponent(query)}&limit=10`,
    detailPath: (slug)=>`/v1/api/phim/${encodeURIComponent(slug)}`,
  ...
  kkphim: {
    ...
    listPath: (page)=>`/danh-sach/phim-moi-cap-nhat?page=${page}`,
    searchPath: (query)=>`/v1/api/tim-kiem?keyword=${encodeURIComponent(query)}&limit=10`,
    detailPath: (slug)=>`/phim/${encodeURIComponent(slug)}`,
...
function normalizeText(value = '') {
  return value.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/\u0111/g, 'd').replace(/\u0110/g, 'd').replace(/[^a-z0-9]+/g, ' ').trim().replace(/\s+/g, ' ');
}
function slugify(value = '') {
  return normalizeText(value).replace(/\s+/g, '-') || 'phim';
}
function canonicalDuplicateTitle(value = '') {
  return normalizeText(value).replace(/\b(18|19|20)\d{2}\b/g, ' ').replace(/\b(tap|ep|episode|phan|season|trailer|vietsub|thuyet minh|long tieng|full|hd|fhd|4k)\b/g, ' ').replace(/\b\d+\b/g, ' ').replace(/\s+/g, ' ').trim();
}
function titleCandidates(record) {
  ...
  ].map((value)=>canonicalDuplicateTitle(String(value || ''))).filter((value)=>value.length >= 6)));
}
...
function isCuratedCatalogMovie(record) {
  const source = `${record?.source_site || ''} ${record?.source_name || ''}`.toLowerCase();
  return source.includes('admin-queer') || source.includes('blvietsub');
}
function canonicalCandidateScore(record) {
  const current = Math.max(Number(record.current_episode || 0), firstEpisodeNumber(String(record.episode_current || '')));
  const total = Math.max(Number(record.total_episodes || 0), totalEpisodeNumber(String(record.episode_total || '')), current);
  return (record.is_published === false ? -10_000 : 0) + total * 20 + current * 4 + sourcePriority(record);
}
function sameMovieByTitle(existing, incoming) {
  ...
  const strictTitles = (record)=>Array.from(new Set([
      record.name,
      record.origin_name,
      record.title_vi,
      record.title_en,
...
function normalizedMovieType(value) {
  ...
  if ([
    ...
    'phim le',
    'phim chieu rap'
  ].includes(type)) return 'single';
  if ([
    'series',
    'tv',
    'phim bo',
    'hoathinh'
  ].includes(type)) return 'series';
  return '';
}
function detailMatchesExpected(expected, detail) {
  const expectedRecord = expected;
  const detailRecord = detail.movie;
...
function isRetryableDatabaseError(error) {
  const record = error && typeof error === 'object' ? error : {};
  const text = dbErrorMessage(error).toLowerCase();
  return record.code === '40P01' || record.code === '40001' || record.code === '55P03' || text.includes('deadlock detected') || text.includes('serialization failure') || text.includes('could not obtain lock');
}
async function runDatabaseMutationWithRetry(operation, attempts = 3) {
...
function isTransientExternalError(value) {
  const text = dbErrorMessage(value).toLowerCase();
  return text.includes('error 521') || text.includes('"status":521') || text.includes('web server is down') || text.includes('cloudflare') || text.includes('timeout') || text.includes('timed out') || text.includes('networkerror') || text.includes('failed to fetch') || text.includes('fetch failed') || text.includes('429') || text.includes('too many requests') || text.includes('rate limit') || text.includes('temporarily unavailable') || text.includes('latest list returned 0 items') || text.includes('from all mirrors') || text.includes('503') || text.includes('502') || text.includes('504');
}
function recordSyncError(stats, message) {
...
function episodeNumber(ep) {
  ...
  const matches = [
    ...text.matchAll(/(\d{1,4})/g)
  ].map((match)=>Number(match[1])).filter(Number.isFinite);
  return matches.length ? Math.max(...matches) : 0;
}
function hasPlayableEpisodeLink(ep) {
  return Boolean(String(ep.link_m3u8 || '').trim() || String(ep.link_embed || '').trim());
}
function firstEpisodeNumber(value = '') {
...
function totalEpisodeNumber(value = '') {
...
function repairConcatenatedEpisodeNumber(current, total) {
...
function getCurrentEpisode(movie, episodes) {
  let hasSourceEpisodeRows = false;
  const sourceTotal = Math.max(totalEpisodeNumber(movie.episode_current || ''), totalEpisodeNumber(movie.episode_total || ''));
...
function hasUsableMovieImage(...records) {
  return records.some((record)=>Boolean(safeProviderImage(record?.thumb_url) || safeProviderImage(record?.poster_url)));
}
...
function detailHasPlayableEpisode(detail) {
  return detail.episodes.some((server)=>(server.server_data || []).some((episode)=>!isTrailerEpisode(episode) && Boolean(String(episode.link_m3u8 || '').trim() || String(episode.link_embed || '').trim())));
}
async function hasPersistedPlayableCoverage(supabase, movieId) {
  ...
  const streamIsSuppressed = (row)=>{
    ...
    const browserManagedException = /https?:\/\/player\.phimapi\.com\/player\//i.test(embed) || /https?:\/\/[^/]*streamc\.xyz\//i.test(embed);
    if (status === 'blocked' && !browserManagedException) return true;
    return status === 'dead' || status === 'failed' && failures >= 3;
  };
  const usableByUrl = new Map();
  const usableByServerSlug = new Map();
  ...
  const matchingHealthState = (row, directKey, embedKey, slugKey)=>{
    const urls = [
      row[directKey],
      row[embedKey]
    ].map(normalizeUrl).filter(Boolean);
  ...
  const legacyRowIsUsable = (row, directKey, embedKey, slugKey)=>{
  ...
  return Boolean((movieEpisodes || []).some((row)=>String(row.source || '').toLowerCase() !== 'hidden' && legacyRowIsUsable(row, 'link_m3u8', 'link_embed', 'slug')) || (episodes || []).some((row)=>legacyRowIsUsable(row, 'link_m3u8', 'link_embed', 'episode_slug')) || streamRows.some((row)=>row.is_active !== false && !streamIsSuppressed(row) && Boolean(normalizeUrl(row.stream_url) || normalizeUrl(row.embed_url))));
}
...
async function readCursorPage(supabase, key, fallbackPage) {
  try {
...
async function writeCursorPage(supabase, key, page) {
  try {
...
async function fetchJsonFromMirrors(provider, path, timeoutMs = 12000) {
  for (const base of provider.bases){
    ...
    /* try next mirror */ } finally{
      clearTimeout(timer);
    }
  }
  return null;
}
function providerListPaths(provider, page) {
  const primary = provider.listPath(page);
  if (provider.sourceSite !== 'phimapi') return [
    primary
  ];
  return Array.from(new Set([
    primary,
...
async function fetchListPage(provider, page) {
...
function isProviderErrorPayload(payload) {
  if (!payload) return false;
  const status = Number(payload.status || payload.error_code || 0);
  const text = JSON.stringify(payload).toLowerCase();
  return status >= 400 || text.includes('error 521') || text.includes('web server is down') || text.includes('cloudflare') || text.includes('origin_down') || text.includes('temporarily unavailable');
}
function parseDetail(payload, fallbackSlug) {
  ...
  const normalized = normalizeVerifiedSeasonNumbering(movie, Array.isArray(episodes) ? episodes : []);
  return {
...
function sourceCandidateScore(item, target) {
  ...
  const itemNames = [
    item.name,
    item.origin_name,
    ...Array.isArray(item.alternative_names) ? item.alternative_names : []
  ].map((value)=>slugify(String(value || ''))).filter(Boolean);
  const exactName = itemNames.some((name)=>targetNames.includes(name));
  const partialName = itemNames.some((name)=>targetNames.some((targetName)=>name.length >= 5 && targetName.length >= 5 && (name.includes(targetName) || targetName.includes(name))));
  ...
  const itemTmdbId = String(item.tmdb?.id || '');
  if (itemTmdbId && target.tmdb_id && itemTmdbId === String(target.tmdb_id)) score += 250;
  return score;
}
async function fetchDetailForTarget(provider, target) {
  const isSafeTargetDetail = (detail)=>{
    const movie = detail.movie;
    const targetTmdb = String(target.tmdb_id || '').trim();
    const detailTmdbObject = movie.tmdb && typeof movie.tmdb === 'object' ? movie.tmdb : null;
...
function moviePayload(provider, detail) {
  const movie = detail.movie;
  ...
  const sourceAdvertisedTotal = Math.max(totalEpisodeNumber(movie.episode_current || ''), totalEpisodeNumber(movie.episode_total || ''));
...
async function findExistingMovie(supabase, payload) {
  const checks = [
    [
      'slug',
      payload.slug
  ...
  for (const [column, value] of checks){
    const { data } = await supabase.from('movies').select('id,slug,name,origin_name,title_vi,title_en,title_zh,title_original,normalized_name,year,type,source_site,source_name,current_episode,total_episodes,episode_current,episode_total,thumb_url,poster_url,tmdb_id,ophim_id,ophim_slug,is_published').eq(column, value).limit(1).maybeSingle();
    if (data?.id) {
      exactMatch = data;
      break;
    }
  ...
  if (title.length >= 3 && year > 0) {
    ...
    const { data } = await supabase.from('movies').select('id,slug,name,origin_name,title_vi,title_en,title_zh,title_original,normalized_name,year,type,source_site,source_name,current_episode,total_episodes,episode_current,episode_total,thumb_url,poster_url,tmdb_id,ophim_id,ophim_slug,is_published').eq('year', year).or(`name.ilike.%${safe}%,origin_name.ilike.%${safe}%,title_vi.ilike.%${safe}%,title_en.ilike.%${safe}%,title_zh.ilike.%${safe}%,title_original.ilike.%${safe}%`).limit(20);
    const matches = (data || []).filter((row)=>!isCuratedCatalogMovie(row)).filter((row)=>sameMovieByTitle(row, payload)).sort((a, b)=>canonicalCandidateScore(b) - canonicalCandidateScore(a));
    const match = matches[0];
    if (match?.id && (!exactMatch || String(match.id) === String(exactMatch.id) || canonicalCandidateScore(match) >= canonicalCandidateScore(exactMatch) + 40)) return match;
  }
  const normalized = String(payload.normalized_name || '').trim();
  if (normalized.length >= 6 && year > 0) {
    const { data } = await supabase.from('movies').select('id,slug,name,origin_name,title_vi,title_en,title_zh,title_original,normalized_name,year,type,source_site,source_name,current_episode,total_episodes,episode_current,episode_total,thumb_url,poster_url,tmdb_id,ophim_id,ophim_slug,is_published').eq('year', year).ilike('normalized_name', normalized).limit(10);
    const match = (data || []).filter((row)=>!isCuratedCatalogMovie(row)).filter((row)=>sameMovieByTitle(row, payload)).sort((a, b)=>sourcePriority(b) - sourcePriority(a))[0];
    if (match?.id && (!exactMatch || String(match.id) === String(exactMatch.id) || canonicalCandidateScore(match) >= canonicalCandidateScore(exactMatch) + 40)) return match;
  }
...
function updatePayloadForExisting(existing, incoming) {
  ...
  const existingTotal = Math.max(Number(existing.total_episodes || 0), totalEpisodeNumber(String(existing.episode_total || '')), totalEpisodeNumber(String(existing.episode_current || '')));
  const current = Math.max(repairConcatenatedEpisodeNumber(Number(existing.current_episode || 0), existingTotal), repairConcatenatedEpisodeNumber(firstEpisodeNumber(String(existing.episode_current || '')), existingTotal));
  const incomingCurrent = Number(incoming.current_episode || 0);
  const incomingTotal = Math.max(Number(incoming.total_episodes || 0), totalEpisodeNumber(String(incoming.episode_total || '')), totalEpisodeNumber(String(incoming.episode_current || '')));
  const existingLooksAheadOfSource = incomingCurrent > 0 && current > incomingCurrent && existingTotal > 0 && incomingTotal > 0 && current > Math.max(existingTotal, incomingTotal);
...
function protectExistingSlug(existing, update) {
...
async function removeConflictingUniqueIdentityFields(supabase, existingId, update) {
  const safeUpdate = {
    ...update
  };
  ...
  for (const [column, rawValue] of checks){
    ...
    if (!value) continue;
    const { data, error } = await supabase.from('movies').select('id').eq(column, value).neq('id', currentId).limit(1).maybeSingle();
    if (!error && data?.id) delete safeUpdate[column];
  }
  return safeUpdate;
}
async function upsertMovie(supabase, provider, detail) {
  ...
  if (existing?.id) {
    ...
    if (existing.is_published === false && existingSource.includes('merged')) {
      return {
        ...
        created: false,
        updated: false,
        hasImage: hasUsableMovieImage(existing, payload),
        retired: true
      };
    }
    ...
    const { error } = await runDatabaseMutationWithRetry(()=>supabase.from('movies').update(update).eq('id', existing.id));
    if (error) {
      if (isDuplicateError(error) && (update.ophim_id || update.ophim_slug)) {
        ...
        const { error: retryError } = await runDatabaseMutationWithRetry(()=>supabase.from('movies').update(retryUpdate).eq('id', existing.id));
  ...
  if (error) {
    if (isDuplicateError(error)) {
      ...
      if (duplicate?.id) {
        ...
        const { error: updateError } = await runDatabaseMutationWithRetry(()=>supabase.from('movies').update(update).eq('id', duplicate.id));
        if (!updateError) return {
          id: String(duplicate.id),
          created: false,
          updated: true,
          hasImage: hasUsableMovieImage(duplicate, payload)
        };
  ...
  return {
    ...
    created: true,
    updated: false,
    hasImage: hasUsableMovieImage(payload)
  };
}
async function upsertMovieEpisodeRowsSafely(supabase, rows, movieSlug) {
  for (const batch of chunks(rows, 500)){
    const { error } = await supabase.from('movie_episodes').upsert(batch, {
    ...
    for (const row of batch){
      ...
      const { data: existing, error: existingError } = await supabase.from('movie_episodes').select('id').eq('movie_id', row.movie_id).eq('episode_number', row.episode_number).ilike('server_name', serverName).limit(1).maybeSingle();
      if (existingError) throw new Error(`movie_episodes duplicate lookup ${movieSlug}: ${existingError.message}`);
      if (existing?.id) {
        const { error: updateError } = await supabase.from('movie_episodes').update({
...
async function upsertStreamRowsSafely(supabase, rows, movieSlug) {
  for (const batch of chunks(rows, 500)){
    ...
    for (const row of batch){
      ...
      const { data: existing, error: existingError } = await supabase.from('streams').select('id').eq('movie_id', row.movie_id).eq('source', source).ilike('server_name', serverName).ilike('episode_slug', episodeSlug).limit(1).maybeSingle();
      if (existingError) throw new Error(`streams duplicate lookup ${movieSlug}: ${existingError.message}`);
      if (existing?.id) {
        const { error: updateError } = await supabase.from('streams').update({
          ophim_id: row.ophim_id,
          server_name: serverName,
...
async function upsertEpisodeRowsSafely(supabase, rows, movieSlug) {
  for (const batch of chunks(rows, 500)){
    ...
    for (const row of batch){
      const serverName = String(row.server_name || '').trim();
      const episodeSlug = String(row.episode_slug || '').trim();
      const { data: existing, error: existingError } = await supabase.from('episodes').select('id').eq('movie_id', row.movie_id).ilike('server_name', serverName).ilike('episode_slug', episodeSlug).limit(1).maybeSingle();
...
async function insertEpisodes(supabase, provider, movieId, detail, labelAsBackup = false) {
  ...
  if (detail.numberingNormalization) {
    ...
    const { data: normalized, error: normalizeError } = await supabase.rpc('normalize_verified_cumulative_season_numbering', {
      p_movie_id: movieId,
    ...
    if (normalized === false) {
      ...
      const { data: storedRows, error: storedRowsError } = await supabase.from('movie_episodes').select('episode_number').eq('movie_id', movieId).eq('source', provider.sourceSite).limit(2000);
      if (storedRowsError) {
        throw new Error(`season numbering verify ${detail.movie.slug}: ${storedRowsError.message}`);
      }
      const storedNumbers = (storedRows || []).map((row)=>Number(row.episode_number || 0)).filter((number)=>Number.isInteger(number) && number > 0);
      const canonicalNumbers = new Set(storedNumbers);
      const alreadyNormalized = storedNumbers.length > 0 && storedNumbers.every((number)=>number <= canonicalTotal) && canonicalNumbers.size === canonicalTotal && Array.from({
        length: canonicalTotal
      }, (_, index)=>index + 1).every((number)=>canonicalNumbers.has(number));
  ...
  const [{ data: existingAdminRows, error: adminSelectError }, { data: existingEpisodeRows, error: episodeSelectError }, { data: existingStreamRows, error: streamSelectError }] = await Promise.all([
    supabase.from('movie_episodes').select('episode_number, server_name, link_m3u8, link_embed').eq('movie_id', movieId),
    supabase.from('episodes').select('episode_number, server_name, episode_slug, link_m3u8, link_embed').eq('movie_id', movieId),
    supabase.from('streams').select('server_name, episode_slug, source, stream_url, embed_url').eq('movie_id', movieId).eq('source', provider.sourceSite)
  ]);
  ...
  for (const ep of parsedEpisodes){
    if (ep.number > 0) {
      ...
      const adminUrlChanged = Boolean(existingAdminRow) && (String(existingAdminRow.link_m3u8 || '') !== ep.linkM3u8 || String(existingAdminRow.link_embed || '') !== ep.linkEmbed);
    ...
    const episodeUrlChanged = Boolean(existingEpisodeRow) && (String(existingEpisodeRow.link_m3u8 || '') !== ep.linkM3u8 || String(existingEpisodeRow.link_embed || '') !== ep.linkEmbed);
    ...
    const streamUrlChanged = Boolean(existingStreamRow) && (String(existingStreamRow.stream_url || '') !== ep.linkM3u8 || String(existingStreamRow.embed_url || '') !== ep.linkEmbed);
...
function structuredFilenameIdentity(filename) {
  ...
  return year > 0 && names.length ? {
    names,
    rawNames,
    year
  } : null;
}
async function quarantineVerifiedForeignEpisodes(supabase, movieId, detail) {
  ...
  const detailTitles = [
    ...
    ...detail.movie.alternative_names || []
  ].map((value)=>normalizeText(String(value || ''))).filter((value)=>value.length >= 3);
  const detailYear = Number(detail.movie.year || 0);
  if (!detailYear || detailTitles.length === 0) return 0;
  const { data, error } = await supabase.from('episodes').select('id,ophim_id,link_m3u8,link_embed,server_data').eq('movie_id', movieId).limit(2000);
  ...
  for (const group of groups.values()){
    ...
    for (const name of group.identity.rawNames){
      ...
      for (const column of [
        ...
        const { data: matches } = await supabase.from('movies').select('id,slug,name,origin_name,title_vi,title_en,title_original,year,source_site,source_name,current_episode,total_episodes,is_published').eq('year', group.identity.year).ilike(column, exact).neq('id', movieId).limit(3);
        for (const match of matches || [])candidates.set(String(match.id), match);
      }
    }
    if (candidates.size === 0) continue;
    const relatedId = [
      ...candidates.values()
    ].sort((a, b)=>canonicalCandidateScore(b) - canonicalCandidateScore(a))[0].id;
  ...
  const deleteIdsByUrls = async (table)=>{
    const idSet = new Set();
    const fields = table === 'streams' ? [
      'stream_url',
      'embed_url'
    ] : [
      'link_m3u8',
  ...
  await supabase.from('catalog_integrity_issues').upsert({
    ...
    movie_id: movieId,
    related_movie_id: relatedMovieIds.size === 1 ? [
      ...relatedMovieIds
    ][0] : null,
    severity: 5,
    confidence: 1,
...
async function writeLog(supabase, stats, elapsedMs, metadata) {
  try {
...
async function runSeoAutomation(supabase, supabaseUrl, serviceKey, cronSecret, changedSlugs) {
...
serve(async (req)=>{
  ...
  if (secret !== cronSecret) return json({
    success: false,
    error: 'Unauthorized'
  }, 401);
  ...
  const includeEpisodes = url.searchParams.get('episodes') === '1' || url.searchParams.get('include_episodes') === '1';
  ...
  try {
    ...
    if (targetMovieId || targetSlug) {
      const { data, error } = await supabase.from('movies').select('id,slug,name,origin_name,title_vi,title_en,title_original,year,type,tmdb_id,ophim_slug,source_site,source_name').eq(targetMovieId ? 'id' : 'slug', targetMovieId || targetSlug).maybeSingle();
    ...
    if (targetSlug || targetMovie) {
      slugs = [
        targetSlug || String(targetMovie?.slug || '')
      ].filter(Boolean);
      pagesWithItems = 1;
    } else {
    ...
    for (const slug of slugs){
      stats.scanned += 1;
      try {
        const fetchedDetail = targetMovie ? await fetchDetailForTarget(provider, targetMovie) : await fetchDetail(provider, slug);
        const expected = candidates.get(slug);
        const detail = fetchedDetail && (!expected || detailMatchesExpected(expected, fetchedDetail)) ? fetchedDetail : null;
        if (!detail) {
          stats.skipped += 1;
        ...
        try {
          ...
          if (includeEpisodes) {
            ...
            const isIndependentProvider = provider.sourceSite === 'phimapi' ? !/(kkphim|phimapi)/.test(primarySource) : !/ophim/.test(primarySource);
            stats.episodesInserted += await insertEpisodes(supabase, provider, result.id, detail, Boolean(targetMovie && isIndependentProvider));
            ...
            if (targetMovie || detailHasPlayableEpisode(detail) && result.hasImage) {
              ...
              await supabase.from('movies').update({
                is_published: persistedPlayableCoverage && result.hasImage
              }).eq('id', result.id);
            }
          }
        } finally{
    ...
    const seoAutomation = !dryRun ? await runSeoAutomation(supabase, supabaseUrl, serviceKey, cronSecret, changedSlugs) : {
      changed_urls: 0,
      caches_cleared: false,
      search_index_refreshed: false,
      google_ping: {
    ...
    await writeLog(supabase, stats, elapsedMs, {
      provider: provider.sourceSite,
      pages,
      limit,
      dry_run: dryRun,
