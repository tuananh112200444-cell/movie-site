...
serve(async (req)=>{
  ...
  try {
    ...
    const target = postId ? buildUpstreamUrl(`${POST_URL.replace(/\/$/, '')}/${encodeURIComponent(postId)}`, {}) : buildUpstreamUrl(FEED_URL, {
    ...
    if (!upstream.ok) {
      ...
      return json({
        error: `BLVietsub upstream returned ${upstream.status}`,
