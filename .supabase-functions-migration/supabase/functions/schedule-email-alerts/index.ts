...
function getCurrentEpisode(movie) {
...
function localVietnamParts(now) {
...
function vietnamLocalToUtcMs(year, month, date, hour, minute) {
  return Date.UTC(year, month, date, hour, minute, 0, 0) - VIETNAM_OFFSET_MS;
}
function getRecurringTarget(movie, now) {
  ...
  let targetMs = vietnamLocalToUtcMs(parts.year, parts.month, parts.date + addDays, releaseTime.hour, releaseTime.minute);
  if (targetMs <= now.getTime()) {
    targetMs += movie.schedule_type === 'weekly' ? 7 * 24 * 60 * 60 * 1000 : 24 * 60 * 60 * 1000;
  }
...
function getAlertCandidate(movie, now) {
...
function isInsideAlertWindow(targetAt, now) {
  const diffMinutes = (targetAt.getTime() - now.getTime()) / 60000;
  return diffMinutes >= ALERT_LEAD_MINUTES - ALERT_WINDOW_MINUTES && diffMinutes <= ALERT_LEAD_MINUTES + ALERT_WINDOW_MINUTES;
}
...
serve(async (req)=>{
  ...
  if (!RESEND_API_KEY || !ADMIN_NOTIFY_EMAIL) {
    return jsonResponse({
      error: 'Missing RESEND_API_KEY or ADMIN_NOTIFY_EMAIL'
    }, 500);
  }
  const now = new Date();
  const supabase = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY);
  const { data: movies, error } = await supabase.from('movies').select('id,slug,name,origin_name,title_vi,title_en,episode_current,episode_total,current_episode,total_episodes,schedule_type,release_time,release_day,schedule_timezone,next_episode_at,next_episode_name,schedule_note,is_published').eq('is_published', true).or('schedule_type.not.is.null,next_episode_at.not.is.null,release_time.not.is.null').limit(500);
  if (error) {
    return jsonResponse({
      error: error.message
    }, 500);
  ...
  for (const movie of movies ?? []){
    ...
    const { data: inserted, error: insertError } = await supabase.from('schedule_email_notifications').insert(insertPayload).select('id').maybeSingle();
    if (insertError) {
    ...
    try {
      ...
      await supabase.from('schedule_email_notifications').update({
        status: 'sent',
