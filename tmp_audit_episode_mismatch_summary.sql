...
select
  ...
  count(*) filter (where card_episode > local_playable_episode and local_playable_episode > 0) as suspected_card_higher_than_local_playable,
  count(*) filter (where card_episode > local_playable_episode and local_playable_episode = 1) as suspected_only_episode_1_loaded,
  count(*) filter (where card_episode > local_playable_episode and local_playable_episode between 2 and 5) as suspected_only_few_episodes_loaded,
  count(*) filter (
    where card_episode > local_playable_episode
      and local_playable_episode > 0
      and lower(coalesce(source_site, '') || ' ' || coalesce(source_name, '')) like '%blvietsub%'
  ) as suspected_blvietsub
from stats;
