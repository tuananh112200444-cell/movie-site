...
as $$
  ...
  query_parts as (
    select
      ...
      slug_query,
      limit_value,
      regexp_split_to_array(token_query, '\s+') as tokens,
      cardinality(regexp_split_to_array(token_query, '\s+')) as token_count,
    ...
    select * from (
      ...
      where m.is_published = true
        ...
        and (
          ...
          or lower(coalesce(m.title_original, '')) ilike '%' || qp.raw_query || '%'
        )
      order by m.updated_at desc nulls last
      limit 140
    ) raw_title_matches
