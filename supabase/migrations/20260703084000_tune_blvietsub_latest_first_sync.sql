...
select cron.schedule(
  ...
  $$
    select net.http_get(
      url := 'https://dzpddbthdeqbkrcjlzap.supabase.co/functions/v1/sync-blvietsub-feed?limit=24&offset=0&page_size=150&refresh_search=0',
...
select cron.schedule(
  ...
  $$
    select net.http_get(
      url := 'https://dzpddbthdeqbkrcjlzap.supabase.co/functions/v1/sync-blvietsub-feed?limit=12&use_cursor=1&cursor_key=blvietsub_sitemap_external&refresh_search=0',
      headers := jsonb_build_object('x-cron-secret', (select decrypted_secret from vault.decrypted_secrets where name = 'CRON_SECRET' limit 1)),
      timeout_milliseconds := 120000
    );
  $$
