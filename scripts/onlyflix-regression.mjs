...
const checks = [
  ...
  [source.includes('const urlChanged') && source.includes("!['is_active', 'health_status', 'failure_count', 'last_error'].includes(key)"), 'Unchanged OnlyFlix URLs must preserve quarantine and accumulated stream health'],
  [source.includes('episode.source_priority || 15'), 'OnlyFlix fallback sources must retain a bounded default priority'],
  [source.includes(".slice(0, 3)"), 'OnlyFlix must expose exactly the bounded Server 3/4/5 fallback set'],
  [source.includes('vidfast\\.(?:pro|vc)|multiembed\\.mov') && source.includes("? 25") && source.includes("? 15") && source.includes(": 10"), 'OnlyFlix Servers 3, 4 and 5 must remain available in deterministic priority order'],
  [source.includes("episode_current: 'Dang cap nhat'") && source.includes('delete update[key]'), 'Unchecked iframe URLs must never promote a movie to Full'],
  ...
  [fs.readFileSync('supabase/functions/movie-detail-proxy/index.ts', 'utf8').includes('const aSecondaryOnlyflix = /vidfast\\.(?:pro|vc)/i'), 'Movie detail must retain OnlyFlix Server 3 as a secondary source'],
  [moviePage.includes('hasPlayableFullMovie') && moviePage.includes("episode_current: 'Full'"), 'Playable full-movie sources must override a stale Trailer catalog label in the UI'],
  [fs.readFileSync('supabase/functions/movie-detail-proxy/index.ts', 'utf8').includes('hasPlayableFullMovie(episodeServers)'), 'Movie detail responses must normalize stale Trailer metadata when a usable Full source exists'],
];
