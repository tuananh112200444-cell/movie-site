...
expect('src/pages/admin-seo-studio/page.tsx', [
  ...
  ['Phát hành SEO tĩnh đang hoạt động', 'static-only publishing does not explain that Pages Worker is optional'],
  ...
  ['Duyệt & xếp hàng 03:30', 'nightly batch publish action is not clear'],
  ['data-kp-release-timing="true"', 'operator cannot choose between nightly and urgent release lanes'],
  ['Ưu tiên trong vài phút', 'urgent SEO release lane is not visible'],
...
expect('supabase/functions/admin-seo-studio/index.ts', [
  ...
  ["SEO_PUBLISH_MODE === 'worker'", 'static-only load still probes and reports the optional Pages Worker as required'],
  ["status: 'queued-static'", 'static-only publication is not returned as an asynchronous queue state'],
  ["status: 'scheduled-nightly'", 'nightly approval still mutates the public profile immediately'],
  ["reason: 'seo_draft_scheduled'", 'nightly approved draft is not persisted in the release queue'],
  ["release_lane: 'nightly'", 'nightly release lane is not persisted'],
