...
for (const [ok, message] of [
  ...
  [edge.includes('function motchillServerName') && edge.includes('#${option.nume}') && edge.includes('Additive-only publication contract') && !edge.includes("not('server_name', 'in', quoted)"), 'Stable server grouping or additive-only source preservation is missing'],
