...
const checks = [
  ...
  [generator.includes('MIN_EXPECTED_UPCOMING_MOVIES = 5') && generator.includes("sitemap-movies-upcoming.xml") && generator.includes('data-kp-upcoming=') && generator.includes('potentialAction: isUpcoming ? undefined'), 'Static generation must publish a fail-closed upcoming sitemap without premature watch schema'],
  ...
  [catalogFunction.includes("has_more: (data ?? []).length === limit") && generator.includes('if (payload.has_more === false) break;'), 'Static catalogue pagination must continue after partially filtered pages'],
  ...
  [worker.includes('getStaticMovieDocument') && worker.includes("'cloudflare-static-movie'") && worker.includes('publicMovieMatch && !isBot(userAgent)'), 'Users and Googlebot must both use built movie HTML before a slow database renderer'],
