...
const asset404 = fs.readFileSync('public/assets/404.html', 'utf8');
...
const resumeWatch = fs.readFileSync('src/hooks/useResumeWatch.ts', 'utf8');
...
const checks = [
  ...
  [lightweightPlayer.includes('const SINGLE_VARIANT_STALL_FAILOVER_MS = 12_000') && lightweightPlayer.includes('hls.levels.length <= 1') && lightweightPlayer.includes('fastFailoverAvailableRef.current') && playerBox.includes('fastFailoverAvailable={hasPositionPreservingFallback}'), 'A stalled single-rendition 1080p source must fail over early only when another seekable source can preserve playback time'],
  ...
  [lightweightPlayer.includes('progressive: true') && page.includes("void import('./components/MovieDetailPlayerSection')") && page.includes('browserManagedHls') && page.includes("import('./components/LightweightHlsPlayer')") && page.includes('runWhenIdle(() =>'), 'Watch routes must stream large first fragments progressively while detail pages prepare first-party player chunks before the viewer presses Watch'],
  ...
  [resumeWatch.includes('const MIN_SECONDS = 15') && resumeWatch.includes('MIN_REMAINING_SECONDS') && page.includes('Date.now() - info.savedAt <= 30 * 60 * 1000') && page.includes('setInitialSeekTime(info.time)'), 'A recently discarded watch tab must silently resume from its checkpoint without requiring five percent progress'],
