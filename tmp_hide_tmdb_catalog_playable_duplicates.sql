...
updated as (
  ...
  set
    ...
    schedule_note = 'Đã có bản phim có nguồn xem: ' || duplicates.playable_slug,
    updated_at = now()
  from duplicates
  where m.id = duplicates.catalog_id
  returning m.id, m.slug, m.name, m.episode_current, duplicates.playable_slug
),
cache_clear as (
  delete from public.home_page_cache
  where id in ('search_index_v1', 'search_index_v2')
  returning id
)
select
  (select count(*) from updated) as hidden_catalog_duplicates,
  coalesce(json_agg(updated order by updated.slug) filter (where updated.id is not null), '[]'::json) as hidden_samples,
  (select count(*) from cache_clear) as cleared_cache_rows
from updated;
