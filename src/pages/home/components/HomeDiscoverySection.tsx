...
export default function HomeDiscoverySection({ onSelect }: HomeDiscoverySectionProps) {
  ...
  return (
    <section
      ref={ref}
      className="reveal home-discovery-band mb-4 overflow-hidden sm:mb-8"
      aria-label="Chọn kho phim và thể loại"
    >
      <div className="grid gap-3 lg:grid-cols-[minmax(220px,0.72fr)_minmax(0,3.28fr)] xl:grid-cols-[minmax(230px,0.68fr)_minmax(0,3.32fr)] 2xl:grid-cols-[minmax(240px,0.64fr)_minmax(0,3.36fr)]">
        ...
        <div className="hidden rounded-xl border border-white/[0.08] bg-white/[0.035] p-2.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.055)] sm:p-3 md:block">
          <div className="mb-2 flex items-center justify-between gap-3 sm:mb-3">
            <div className="min-w-0">
              <h3 className="truncate text-sm font-black text-white sm:text-base lg:text-lg">Thể loại phổ biến</h3>
              <p className="mt-0.5 hidden text-xs text-white/35 md:block">Chọn nhanh đúng gu phim muốn xem</p>
            </div>
            <Link
              to="/filter"
          ...
          <div className="grid grid-cols-3 gap-1.5 sm:gap-2 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
            {GENRE_LIST.slice(0, 12).map((g, index) => (
              <Link
                ...
                className={`group relative min-h-[54px] overflow-hidden rounded-lg border p-2 transition-all duration-300 hover:-translate-y-0.5 hover:bg-white/[0.06] sm:min-h-[70px] sm:p-2.5 xl:min-h-[76px] ${index >= 9 ? 'hidden sm:block' : ''}`}
                style={{
                  '--g-color': g.color,
                  background: g.bg,
