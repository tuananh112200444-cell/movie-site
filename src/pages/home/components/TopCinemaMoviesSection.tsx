...
function isVerifiedVietnamMovie(movie: MovieItem): boolean {
  const ep = (movie.episode_current ?? '').toLowerCase().trim();
  return ep !== 'trailer' && (movie.country ?? []).some((country) => country?.slug === 'viet-nam');
}

function normalizeVietnamMovies(items: MovieItem[]): MovieItem[] {
  const seen = new Set<string>();
  return items
    .filter(isVerifiedVietnamMovie)
    .filter((movie) => {
    ...
    .sort((left, right) => {
      const rightTime = Date.parse(right.modified?.time ?? '') || 0;
      const leftTime = Date.parse(left.modified?.time ?? '') || 0;
      return rightTime - leftTime || Number(right.year || 0) - Number(left.year || 0);
    });
}
...
const EMPTY_VIETNAM_MOVIES: MovieItem[] = [];

export default function VietnamMoviesSection({ fallbackMovies = EMPTY_VIETNAM_MOVIES }: { fallbackMovies?: MovieItem[] }) {
  const normalizedFallback = useMemo(() => normalizeVietnamMovies(fallbackMovies), [fallbackMovies]);
  const [movies, setMovies] = useState<MovieItem[]>(normalizedFallback);
  const [loading, setLoading] = useState(normalizedFallback.length === 0);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    if (normalizedFallback.length > 0) setMovies((current) => current.length > 0 ? current : normalizedFallback);
    setLoading(normalizedFallback.length === 0);

    fetchVietnamMovies(controller.signal)
      .then((items) => {
        if (cancelled) return;
        setMovies(items);
      })
      .catch(() => {
        if (cancelled) return;
        setMovies((current) => current.length > 0 ? current : normalizedFallback);
      })
      .finally(() => {
  ...
  return (
    <div className="mb-7 md:mb-10 home-section-surface">
      ...
      <div className="home-rail-frame relative group/slider">
        ...
        <button
          onClick={() => scroll('left')}
          aria-label="Cuộn danh sách phim Việt Nam sang trái"
          className={`
        ...
        <button
          ...
          aria-label="Cuộn danh sách phim Việt Nam sang phải"
          className={`
...
function CinemaCard({ movie, rank }: CinemaCardProps) {
  ...
  const rankStyle = rankConfig[rank - 1] ?? {
    bg: 'bg-black/70',
    text: 'text-white/75',
      ...
      >
        ...
        <div
          className={`
            ...
            ${isTop3
              ? 'border-amber-500/20 md:hover:border-amber-400/55 md:hover:shadow-2xl md:hover:shadow-amber-500/15'
          ...
          <div className="movie-art-frame movie-art-frame--portrait aspect-[2/3] relative overflow-hidden">
            ...
            <div className="absolute top-2 left-2 z-[15]">
              <div
                ...
                aria-label={`Vị trí ${rank}`}
              >
                {rankStyle.icon ?? <span>{rank}</span>}
              </div>
            </div>
            ...
            {/* ── Vietnam label (top-right, below quality) ── */}
            ...
            {/* ── Real update time, without invented viewer counts ── */}
            <div className="absolute bottom-2 right-2 z-[12] hidden sm:flex flex-col items-end gap-0.5">
              {mTime && (
                <span className="hidden rounded-md bg-black/60 px-1.5 py-0.5 text-[9px] text-green-400/85 md:block">
                  {timeAgo(mTime)}
                </span>
              )}
            </div>
            ...
            {isTop3 && (
              <div className="absolute inset-x-0 top-0 z-[10] h-[2px] overflow-hidden bg-amber-500/20">
                <div className="h-full w-full bg-gradient-to-r from-transparent via-amber-300/70 to-transparent opacity-80 transition-opacity duration-500 md:opacity-40 md:group-hover:opacity-100" />
              </div>
            )}
