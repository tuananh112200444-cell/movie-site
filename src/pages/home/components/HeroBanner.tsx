...
function HeroBanner({ movies, loading }: HeroBannerProps) {
  ...
  return (
    <div className="relative w-full overflow-hidden" style={{ aspectRatio: '16/6', minHeight: 'clamp(240px, 62vw, 280px)' }}>
      ...
      <div className="absolute inset-0 flex items-center px-3 pt-16 pb-10 sm:px-8 sm:pt-20 sm:pb-6 md:px-12 md:pt-24 md:pb-4 lg:px-8">
        <div key={contentKey} className="hero-content-enter mx-auto flex w-full max-w-[1760px] items-center justify-between gap-3 md:gap-8 2xl:px-2">
          ...
          <div className="max-w-lg flex-1 xl:max-w-2xl 2xl:max-w-3xl">
            ...
            <div className="hero-badges-enter flex items-center gap-1.5 md:gap-2 mb-2 md:mb-3 flex-wrap">
            ...
            <div className="hero-badges-enter flex items-center gap-1.5 md:gap-2 mb-3 md:mb-4 flex-wrap">
              {active.quality && (
            ...
            <div className="hero-cta-enter flex items-center gap-2 md:gap-3">
              <Link to={movieDetailUrl(active.slug)}
                className="group flex items-center gap-2 bg-red-500 hover:bg-red-600 text-white text-xs md:text-sm font-bold px-3 sm:px-6 py-2 sm:py-3 rounded-xl transition-all duration-200 hover:scale-[1.04] active:scale-[0.97] whitespace-nowrap shadow-lg shadow-red-500/30">
              ...
              <Link to={activeDetailHref}
                className="flex items-center gap-1.5 md:gap-2 bg-white/[0.08] hover:bg-white/[0.14] text-white text-xs md:text-sm font-semibold px-3 md:px-5 py-2 sm:py-2.5 rounded-xl transition-all duration-200 border border-white/[0.12] hover:border-white/25 hover:scale-[1.02] active:scale-[0.97] whitespace-nowrap">
                <i className="ri-information-line text-white/60 text-xs" />Chi Tiết
              </Link>
              <button
                onClick={(e) => { e.preventDefault(); toggle(active); }}
                className={`flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 rounded-xl transition-all duration-200 border cursor-pointer whitespace-nowrap active:scale-[0.93] ${
                  favored
      ...
      <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5 md:hidden" role="tablist" aria-label="Điều hướng slide">
        {featured.map((m, i) => (
          <button key={i} onClick={() => handleSelectSlide(i)}
            ...
            className={`rounded-full transition-all duration-300 cursor-pointer active:scale-90 ${
              i === activeIndex ? 'w-5 h-1.5 bg-red-500' : 'w-1.5 h-1.5 bg-white/25 active:bg-white/50'
            }`} />
        ))}
      </div>
    </div>
