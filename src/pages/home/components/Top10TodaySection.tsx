...
import { fetchNewMovies, getOptimizedImageUrl } from '../../../services/movieApi';
...
function MobileTop10Card({ movie, rank }: CardProps) {
  ...
  return (
    <Link
      to={`/phim/${encodeURIComponent(movie.slug || '')}`}
      className={`${HOME_POSTER_ITEM_CLASS} group cursor-pointer`}
    >
      <div className="relative overflow-hidden rounded-lg bg-white/5 aspect-[2/3]" style={{ border: `1px solid ${rc.border}` }}>
        {!imgLoaded && !imgError && <div className="absolute inset-0 animate-pulse bg-white/5" />}
        {imgError && (
          <div className="absolute inset-0 flex items-center justify-center bg-[#1a1d27] z-[1]">
            <i className="ri-image-line text-white/20 text-2xl" />
        ...
        <span
          className="absolute left-1.5 top-1.5 text-2xl font-black leading-none"
          style={{
            color: rc.numColor,
...
function MobileSkeletonCard() {
  return (
    <div className={HOME_POSTER_ITEM_CLASS}>
      ...
      <div className="mt-1 h-2.5 w-2/3 rounded bg-white/5 animate-pulse" />
    </div>
  );
}
...
export default function Top10TodaySection({ initialMovies = [], loading: parentLoading = false }: Top10TodaySectionProps) {
